...
#rezero time
data_rezeroed <- data %>%
  rezero_times() # right now, rezeroing on trial_num (NOT event_num)
...
Issues to deal with:
- some timepoints are not being read in correctly (e.g. in careylabHarvard_adults_xy_timepoints), apparently because these are too large (10^12)
- some residual import/ processing issues with some datasets
-- careylabHarvard_adults_xy_timepoints has some participant ids under lab_id
