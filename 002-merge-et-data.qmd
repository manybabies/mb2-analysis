...
et_adult_labids <- unique(filter(data, age_cohort == "adults")$lab_id)
et_toddler_labids <- unique(filter(data, age_cohort == "toddlers")$lab_id)
...
data |>
  ...
  summarise(min_t = min(t_norm, na.rm=TRUE), 
            ...
            mean_diff = round(mean(diff(t_norm), na.rm=TRUE))) 
...
Why are there some event numbers that go to 22?
...
```{r}
hist(data$event_num, breaks = 0:22)
```
