...
correct_ids <- function(df) {
  ...
  df <- df %>%
    mutate(participant_id = case_when(
      age_cohort == 'adult' & lab_id == 'careylabHarvard' ~ paste0('careylabHarvard_', participant_id),
      age_cohort == 'adult' & lab_id == 'MEyeLab' ~ sub("\\.edf$", "", participant_id),
      age_cohort == 'toddler' & lab_id == 'careylabHarvard' ~ sub("careylabharard", "careylabHarvard", participant_id),
      age_cohort == 'toddler' & lab_id == 'UIUCinfantlab' ~ paste0(substr(participant_id, 1, 2), "0"),
      TRUE ~ participant_id
    ))
