...
```{r}
error_ids_adults <- adult_demo_minimal %>%
  filter(session_error == 'error') %>%
  select(participant_id, lab_id, session_error, session_error_info, session_error_notes)

error_ids_toddlers <- toddler_demo_minimal %>%
  filter(session_error == 'error') %>%
  select(participant_id, lab_id, session_error, session_error_info, session_error_notes)

data <- data %>% anti_join(error_ids_adults, by = c('lab_id', 'participant_id')) %>% anti_join(error_ids_toddlers, by = c('lab_id', 'participant_id'))

# TODO we need to spin the error ids out into a file that gets passed through to the exclusion pipeline
# TODO get the know missings read.csv(here("metadata","known_missing.csv")) and see what data is missing beyond the session errors
...
data_merged <- left_join(data, adult_demo_minimal, 
                         by = c("age_cohort","lab_id","participant_id")) |>
  left_join(toddler_demo_minimal, 
            by = c("age_cohort","lab_id","participant_id"))
```

Remove columns that we don't think we're going to need - just for sanity.

START HERE - THIS MERGE IS CREATING MULTIPLE FAM_ERROR COLUMNS SO SOMETHING IS WRONG
