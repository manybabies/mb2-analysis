...
# could use some cleanup to eliminate redundancy

pilot_ids_adults <- adult_demo_minimal %>%
  filter(pilot == "yes") %>%
  select(participant_id, lab_id) %>%
  mutate(age_cohort = "adults")

pilot_ids_toddlers <- toddler_demo_minimal %>%
  filter(pilot == "yes") %>%
  select(participant_id, lab_id) %>%
  mutate(age_cohort = "toddlers")

pilot_ids_all <- pilot_ids_toddlers %>%
  rbind(pilot_ids_adults)
...
error_ids_all <- error_ids_toddlers %>%
  rbind(error_ids_adults)
