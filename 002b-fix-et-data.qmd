...
MZ comment: I'm confused about idea (a). t should "reset" every trial based on our resampling strategy. I think this might be outdated?
```{r}
ppt_id_unique <- data |> 
  mutate(t_lag = lag(t),
...
# this misses the very first participant! So need to add 1 in the assert call
...
# starcalib occurs twice for some participants
assert_that(all(filter(media_unique, media_name!="star_calib")$n_media == 1)) 
```
