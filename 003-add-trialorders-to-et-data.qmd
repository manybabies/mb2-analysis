...
# data <- data %>% 
 # filter(!(lab_id %in% c(
 # 'mpievaCCP' # TODO: for some reasons, all participants appear twice? This is due to mpievaccp and mecdmpihcbs using the same prefix in their participant id --> fixed through unique participants_lab-id!
 # ))) %>% 
 # filter(
 #     !(participant_id == 'UVA_127-toddlers' & lab_id == 'babylabAmsterdam') # & # TODO: This participant appears twice, (wait, didn't we try to fix this?)
    #!(participant_id == 'RU_017-toddlers' & lab_id == 'babylabNijmegen') # duplicate - fixed
