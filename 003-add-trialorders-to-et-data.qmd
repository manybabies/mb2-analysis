...
data <- data %>% 
  filter(!(lab_id %in% c(
  'mpievaCCP', # TODO: for some reasons, all participants appear twice?
  'gertlabLancaster' # TODO: the media names are broken for some reason
  ))) %>% filter(
    !(participant_id == 'MPI_022-toddlers' & lab_id == 'mecdmpihcbs') & # this participant has 6 fam trials (remove?)
      !(participant_id == 'UVA_127-toddlers' & lab_id == 'babylabAmsterdam') & # TODO: This participant appears twice, (wait, didn't we try to fix this?)
    !(participant_id == 'RU_017-toddlers' & lab_id == 'babylabNijmegen') & # duplicate
      !(participant_id == 'SkidLSDLab_009-adults' & lab_id == 'SkidLSDLab'), # has 10 trials?
    ) 
