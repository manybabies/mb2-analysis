...
# TODO: temporary filter to filter out participants that have seen any media twice (according to the data, investigate this)
data <- data %>% 
  filter(!(lab_id %in% c(
  'labUNAM' # various participants affected
  ))) %>% 
  filter(!(participant_id %in% c('UoO231', 'SkidLSDLab_009')))
