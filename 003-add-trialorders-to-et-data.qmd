...
```{r}
# TODO these labs/participants are currently broken - investigate this
# (lab_id, participant_id, media_name) does not uniquely identify a trial_number
data <- data %>% 
  filter(!(lab_id %in% c(
  'babylabNijmegen', # TODO: participants are not unique within the lab - only unique within the age cohorts, at which point in the pipeline do we fix this?
  'labUNAM' # TODO: same, participants are not unique within the lab - only unique within the age cohorts, at which point in the pipeline do we fix this?
  ))) %>% filter(
    !(participant_id == 'MPI_022' && lab_id == 'mecdmpihcbs') && # this participant has 6 fam trials (restarted testing and abandoned?)
      !(participant_id == 'UVA_127' && lab_id == 'babylabAmsterdam'), # TODO: This participant appears twice, INVESTIGATE THIS
    !(participant_id == 'UoO231' && lab_id == 'oxfordBabylab'), # TODO: This participant appears twice, INVESTIGATE THIS
    !(participant_id == 'SkidLSDLab' && lab_id == 'SkidLSDLab_009') # TODO: This participant appears twice, INVESTIGATE THIS
    ) 
