...
dataset_specs <- read.csv(here('metadata', 'dataset_sheet.csv')) %>%
  select(data_id, data_type, screen_dimensions, point_zero) %>%
...
data_with_aois <- data_with_aois %>%
  filter(!(lab_id %in% c("CorbitLab", "MEyeLab", "UIUCinfantlab", "babylabMPIB", "babylabPrinceton", "babylingOslo", "kidsdevUniofNewcastle", "sociocognitivelab", "tauccd", "unicph")))
