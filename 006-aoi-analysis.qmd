...
summarize_participant_familiarization <- fam_data %>%
  ...
  mutate(familiarization_trial_num = rank(trial_num, ties.method = "first"))
```
...
#set the prior
priors <-c(
  prior(uniform(0, 1), lb=0,ub=1,class = Intercept), #uniform distribution for intercept
  prior(normal(0, .1), class = b), # normal distribution for fixed-effect coefficients
  prior(normal(0, .05), class = sd), #normal distribution for sd/ random effects
