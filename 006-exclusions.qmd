...
data_preprocessed <- data_preprocessed %>%
  left_join(combined_demo) #by=join_by(participant_lab_id, lab_id, participant_id, age_cohort))

#filter out participants that fall outside of the age range
data_preprocessed <- data_preprocessed %>%
...
data_preprocessed <- data_preprocessed %>%
    filter(is.na(man_exclude)|man_exclude==FALSE)
```
...
data_preprocessed <- left_join(data_preprocessed, trial_level_errors_demographics_lab_reported)
...
summarized_valid_familiarization_trials <- data_preprocessed %>%
  ...
  filter(trial_error_valid_looking == "no" & trial_error_lab_reported == "no") %>%
  distinct(participant_lab_id,lab_id,age_cohort,participant_id,participant_trial_id,trial_num) %>%
...
summarized_valid_first_test_trials <- data_preprocessed %>%
  ...
  filter(trial_num==5) %>%
  distinct(participant_lab_id,lab_id,age_cohort,condition,participant_id,participant_trial_id,trial_num,trial_error_valid_looking, trial_error_lab_reported) %>%
  mutate(
    valid_first_test_trial = case_when(
      (trial_error_lab_reported == "yes" | trial_error_valid_looking == "yes") ~ 0,
      (trial_error_lab_reported == "no" & trial_error_valid_looking == "no") ~ 1,
      TRUE ~ NA
    )
  ) %>%
  #remove columns that vary within participants, to make joining at the participant level possible
...
participant_trial_error_info <- data_preprocessed %>% 
  distinct(participant_lab_id, trial_based_error, trial_based_error_type)
...
overview_trial_based_errors <- data_preprocessed %>%
  distinct(participant_lab_id,lab_id,age_cohort,participant_id,trial_based_error,trial_based_error_type) %>%
  group_by(age_cohort) %>%
  summarize(
    total_N = n(),
...
n_adults_included <- d_participants %>% filter(age_cohort=="adults") %>% summarise(n = length(unique(participant_lab_id)))
n_toddlers_included <- d_participants %>% filter(age_cohort=="toddlers") %>% summarise(n = length(unique(participant_lab_id)))
...
n_adults_collected <- combined_demo %>% 
  filter(age_cohort=="adults") %>% 
  group_by(lab_id) %>% 
  summarise(N_collected = length(unique(participant_lab_id)))
...
lab_stats_adults_female <- lab_stats_adults_included %>% group_by (lab_id, participant_gender) %>%
  summarise(N_female = length(unique(participant_lab_id))) %>% filter(participant_gender=="woman") %>% select(-participant_gender)
  
lab_stats_adults_included <- lab_stats_adults_included %>% 
  group_by (lab_id) %>%
  summarise(N_included = length(unique(participant_lab_id)))
...
lab_stats_toddlers_female <- lab_stats_toddlers_included %>%
  ...
  summarise(N_female = length(unique(participant_lab_id))) %>%
...
Our final dataset will consist of `r n_before_exclusion` participants, with an overall exclusion rate of XYZ% (toddlers: XYZ%, adults: XYZ%). Tables 1 A. and B. show the distribution of included participants across labs, eye-tracking methods, and ages. A final sample of `r sum(lab_stats_toddlers_included$N_included)` toddlers (XX% female) that will have been tested in `r n_labs_toddlers` labs (mean lab sample size = `r mean(lab_stats_toddlers_included$N_included)`, SD =`r sd(lab_stats_toddlers_included$N_included)`, range: `r min(lab_stats_toddlers_included$N_included)` - `r max(lab_stats_toddlers_included$N_included)`) will be analyzed. The average age of toddlers in the final sample will be `r round(age_stat_toddlers$mean, 2)` months (SD: `r round(age_stat_toddlers$sd, 2)`, range: `r round(age_stat_toddlers$min, 2)` - `r round(age_stat_toddlers$max, 2)`). The final sample size of included adults will be N = `r sum(lab_stats_adults_included$N_included)` (XX% female), tested in `r n_labs_adults` labs (mean lab sample size = `r mean(lab_stats_adults_included$N_included)`, SD = `r sd(lab_stats_adults_included$N_included)`, range: `r min(lab_stats_adults_included$N_included)` - `r max(lab_stats_adults_included$N_included)`). Their mean age will be `r round(age_stat_adults$mean, 2)` years (SD: `r round(age_stat_adults$sd, 2)`, range: `r round(age_stat_adults$min, 2)` - `r round(age_stat_adults$max, 2)`).
