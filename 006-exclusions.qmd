...
# TO-DO: integrate with sufficient looking data error
# lab-reported error takes precedence
```
...
names(data_preprocessed)[names(data_preprocessed) == 'trial_error'] <- 'trial_error_valid_looking'
data_preprocessed <- data_preprocessed %>%
  select(-trial_error_type)
...
summarized_valid_familiarization_trials <- data_preprocessed %>%
  ...
  filter(trial_error_valid_looking == "no") %>%
...
summarized_valid_first_test_trials <- data_preprocessed %>%
  ...
  filter(trial_num==5) %>%
  distinct(participant_lab_id,lab_id,age_cohort,condition,participant_id,participant_trial_id,trial_num,trial_error_valid_looking) %>%
  mutate(
    valid_first_test_trial = case_when(
      trial_error_valid_looking == "yes" ~ 0,
      trial_error_valid_looking == "no" ~ 1,
      TRUE ~ NA
    )
  ) %>%
  #remove columns that vary within participants, to make joining at the participant level possible
...
participant_trial_errors <- data_preprocessed %>%
  mutate(
    ...
    trial_based_error_type = case_when(
      trial_error_lab_reported == "yes" ~ "0",
...
```{r}
data_preprocessed <- participant_trial_errors %>% 
  filter(participant_trial_errors$trial_based_error=="no")
...
d_participants <- participant_trial_errors %>% distinct(participant_lab_id, trial_based_error, trial_based_error_type)
