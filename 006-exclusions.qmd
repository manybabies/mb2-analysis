...
trial_level_errors_demographics <- combined_demo %>%
  select(lab_id,participant_lab_id,participant_id,trial_error_demo_cols) %>%
  ...
  mutate(
    ...
    trial_error_type = case_when(
      error == "noerror" ~ NA,
      error == "error" ~ "lab reported trial error"
    )
  )
...
summarized_valid_first_test_trials <- data_preprocessed %>%
  ...
  filter(trial_num==5) %>%
  distinct(participant_lab_id,lab_id,age_cohort,condition,participant_id,participant_trial_id,trial_num,trial_error) %>%
  mutate(
