...
data_preprocessed <- arrow::open_dataset(here(INTERMEDIATE_FOLDER, INTERMEDIATE_005_par), format = "parquet")
...
participants_demo_only <- setdiff(combined_demo$participant_lab_id, et_ids)
...
participants_et_only <- setdiff(et_ids,combined_demo$participant_lab_id)
#validate
...
participants_with_only_calibration_et <- setdiff(
  unique_et_participants_trials %>% distinct(participant_lab_id) %>% pull(participant_lab_id),
...
valid_media_names <- data_preprocessed %>%
  distinct(media_name) %>%
  pull(media_name)

assert_that(!any(unique(filter(manual_exclusions,!(media_name %in% valid_media_names))$participant_lab_id) %in%
                   (data_preprocessed %>%
                   distinct(participant_lab_id) %>%
                   pull(participant_lab_id))))
...
assert_that(sum(is.na(unique_et_participants_trials %>% pull(sufficient_fam_trials)))==0)
```
...
```{r}
arrow::write_dataset(data_preprocessed, 
                     here(INTERMEDIATE_FOLDER, INTERMEDIATE_006b_par),
                     partitioning = "lab_id",
...
arrow::write_dataset(data_preprocessed_post_exclusions,
                     here(INTERMEDIATE_FOLDER, INTERMEDIATE_006_par),
                     partitioning = "lab_id",
                      existing_data_behavior="overwrite")
