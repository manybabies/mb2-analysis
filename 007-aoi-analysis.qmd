...
```{r}
#plot average proportion looking
overall_p <- ggplot(summarize_participant_test_first_trial, aes(x=condition, y=prop_exit,color=condition))+
  #geom_violin()+
  #geom_boxplot()+
...
ggplot(summarize_overall_test_first_look_first_trial,aes(condition,prop_target_first_look,color=condition))+
  geom_errorbar(aes(ymin=lower_ci,ymax=upper_ci),width=0)+
  geom_point(size = 3) +
  geom_hline(yintercept=0.5,linetype="dashed")+
  facet_wrap(~age_cohort)+
...
brms::bayes_factor(bm_fam_first_look_toddlers, null_fam_first_look_toddlers)
```
...
brms::bayes_factor(bm_fam_first_look_adults, null_fam_first_look_adults)
```
...
bm_first_look_toddlers <- brm(
  ...
  seed = 123,
  sample_prior=TRUE)
summary(bm_first_look_toddlers)
prior_summary(bm_first_look_toddlers)

t.test(test_first_look_first_trial$correct_first_look[test_first_look_first_trial$age_cohort=="toddlers" & test_first_look_first_trial$condition=="knowledge"], alternative = "two.sided", mu=0.5)
```
...
```{r}
h_first_look_toddlers <- hypothesis(bm_first_look_toddlers, "condition_c = 0", class="b")
...
```{r}
null_first_look_toddlers <-  update(bm_first_look_toddlers, formula = ~ .-condition_c)
summary(null_first_look_toddlers)
brms::bayes_factor(bm_first_look_toddlers, null_first_look_toddlers)
```
...
bm_first_look_adults <- brm(
  ...
  seed = 123,
  sample_prior=TRUE)
summary(bm_first_look_adults)
prior_summary(bm_first_look_adults)
```
...
```{r}
h_first_look_adults <- hypothesis(bm_first_look_adults, "condition_c = 0", class="b")
h_first_look_adults
...
```{r}
null_first_look_adults <-  update(bm_first_look_adults, formula = ~ .-condition_c)
summary(null_first_look_adults)
brms::bayes_factor(bm_first_look_adults, null_first_look_adults)
```
...
#correlation between familiarization and test
corrlelation_fam_test <- summarize_participant_test_first_trial %>%
  select(age_cohort,condition,fam_prop_exit,prop_exit) %>%
  group_by(age_cohort,condition) %>%
  nest() %>%
  #use map to compute correlation
  summarize(correlation=map(data,corrr::correlate))
...
cor.test(corrlelation_fam_test$fam_prop_exit[corrlelation_fam_test$condition=="knowledge" & corrlelation_fam_test$age_cohort=="toddlers"], corrlelation_fam_test$prop_exit[corrlelation_fam_test$condition=="knowledge"& corrlelation_fam_test$age_cohort=="toddlers"])

cor.test(corrlelation_fam_test$fam_prop_exit[corrlelation_fam_test$condition=="ignorance" & corrlelation_fam_test$age_cohort=="toddlers"], corrlelation_fam_test$prop_exit[corrlelation_fam_test$condition=="ignorance" & corrlelation_fam_test$age_cohort=="toddlers"])
```
