...
INTERMEDIATE_006 <- "005-aoi-data.Rds"
...
rle_test_data <- test_data %>%
  ...
  group_by(age_cohort,participant_lab_id,lab_id,participant_id,participant_trial_id, trial_num,condition) %>%
...
bm_first_look_toddlers <- brm(
  correct_first_look ~ 1+condition+(1+condition|lab_id),
...
bm_first_look_toddlers %>%
  spread_draws(b_conditionknowledge) %>%
  mean_hdi(.width = 0.95)
```
...
```{r}
h_first_look_toddlers <- hypothesis(bm_first_look_toddlers, "conditionknowledge = 0", class="b")
...
```{r}
null_first_look_toddlers <-  update(bm_first_look_toddlers, formula = ~ .-condition)
...
```{r}
h_first_look_adults <- hypothesis(bm_first_look_adults, "conditionknowledge = 0", class="b")
h_first_look_adults
...
```{r}
null_first_look_adults <-  update(bm_first_look_adults, formula = ~ .-condition)
