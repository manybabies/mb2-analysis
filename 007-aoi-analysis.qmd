...
rle_test_data <- test_data %>%
  ...
  group_by(participant_lab_id,lab_id,participant_id,participant_trial_id, trial_num) %>%
...
get_first_look <- function (rle_data, SAMPLING_RATE = 40, MINIMUM_ANTICIPATORY_LOOK_MS = 150) {
  ...
  if (sum(rle_data$values!="NA_NA") == 0) {
    return(tibble(
      first_look = NA,
      rt = NA,
      shift_type = NA))
  ...
  #any requirements for the first look location?
  ...
  #remove the starting look location
  #i.e. remove the first row
  rle_data_post_first_look <- rle_data[-1,]
  
  #first landing has to be post the initial look location
  first_landing <- rle_data_post_first_look$values[
                                     rle_data_post_first_look$values %in% c("target_exit","distractor_exit") &
                                     rle_data_post_first_look$lengths >= min_look_length][1]
  ...
  first_landing_idx <- which(rle_data_post_first_look$values == first_landing & rle_data_post_first_look$lengths >= min_look_length)[1]+1
  
  values_before_first_landing <- rle_data$lengths[1:(first_landing_idx-1)]
  
  shift_type <- paste0(onset_aoi,"_TO_",first_landing)
  shift_type_all <- paste(rle_data$values[1:first_landing_idx],collapse="_TO_")
