...
get_first_look <- function (rle_data, SAMPLING_RATE = 40, MINIMUM_ANTICIPATORY_LOOK_MS = 150) {
  ...
  first_shift_landing_rle <- rle_data_idx |> 
    filter(idx_skip_nas != 1, # first shift landing is post the initial look location, not counting NAs (or do we count NAs here?)
  ...
  first_look_rt <- ((rle_data_idx |> 
    ...
    pull(lengths) |> sum()) + 1) * (1000/SAMPLING_RATE)
