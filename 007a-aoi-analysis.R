...
overall_p_test <- ggplot(summarize_participant_test_first_trial, aes(x=condition, y=prop_exit,color=condition))+
  ...
  geom_half_boxplot(data=filter(summarize_participant_test_first_trial,condition=="ignorance"),side="l",nudge=0.15,width=0.4,errorbar.draw=F)+
  geom_half_boxplot(data=filter(summarize_participant_test_first_trial,condition=="knowledge"),side="r",nudge=0.15,width=0.4,errorbar.draw=F)+
...
adults_prop <- ggplot(filter(summarize_participant_test_first_trial,N_exit>=5&age_cohort=="adults"), aes(x=condition, y=prop_exit,color=condition))+
  ...
  ylab("Proportion Looking to Exit\n(Anticipatory Window, First Trial)")
adults_prop
ggsave(here(plot_path,"adults_proportion_first_trial_target_exit_looking.png"),bg="white", width = 16, height = 10)
...
bm_aoi_toddlers <- brm(prop_exit ~ 1+condition_c*age_mo_c+(1+condition_c*age_mo_c|lab_id),
         family=gaussian,
         prior = priors,
         save_pars = save_pars(all = TRUE),
...
mss_timecourse_test <- test_data %>%
  ...
  group_by(age_cohort, participant_lab_id,participant_id, participant_trial_id, 
...
ggplot(filter(ms_timecourse_test_long,t_norm_downsampled<=0),       
         aes(x = t_norm_downsampled, y = mean, 
             col = aoi_type, lty = trial_number_name)) + 
  # geom_point(alpha = .1) +
  geom_line() + 
  geom_ribbon(aes(ymin = mean-se, ymax = mean+se, fill = aoi_type), alpha = .2,color=NA) +
...
ms_timecourse_test_outcome <- mss_timecourse_test |>
  separate(media_name,into=c("condition_name","location_pattern","exit_location","outcome"),remove=FALSE) |> 
...
ggplot(filter(ms_timecourse_test_outcome_long,!first_trial),       
         aes(x = t_norm_downsampled, y = mean, 
...
ms_fam <- mss_fam |>
  ...
  summarize(target_exit=mean(on_target, na.rm=TRUE),
...
# TO DO: add binomial test

ggplot(filter(d_anticipatory_fam,!is.na(first_look)),aes(trial_num,fill=first_look))+
  geom_bar(position="fill")+
  facet_wrap(~age_cohort)

ggplot(summarize_overall_familiarization_first_look,aes(trial_num,prop_target_first_look,color=trial_num))+
...
bm_fam_first_look_toddlers <- brm(first_look ~ 1+familiarization_trial_num_4+(1+familiarization_trial_num_4|lab_id)+(1+familiarization_trial_num_4|participant_lab_id),
...
bm_test_first_look_toddlers <- brm(
  correct_first_look ~ 1+condition_c*age_mo_c+(1+condition_c*age_mo_c|lab_id),
  family=bernoulli(link = "logit"),
  prior = priors,
  save_pars = save_pars(all = TRUE),
...
figure_fam_test_p_fl <- cowplot::plot_grid(overall_p_fam +
