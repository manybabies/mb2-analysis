...
fam_m_comparison_PTL_toddlers <- brms::bayes_factor(
  bm_fam_aoi_toddlers, bm_fam_aoi_toddlers_null
)

saveRDS(fam_m_comparison_PTL_toddlers,
...
```{r}
figure_prop_lab <- ggarrange(
  adults_prop, kids_prop, 
  ncol = 1, nrow = 2
)

figure_prop_lab <- cowplot::plot_grid(
  adults_prop +
    theme(
...
ggsave(here(plot_path, "figure_prop_lab_both.png"),
  bg = "white", width = 16, height = 20
)
```
...
Combine Loess and age bin plots

```{r}
figure_prop_age_loess_bin <- ggarrange(
  kid_prop_by_age_loess, kids_prop_age_bin, 
  ncol = 1, nrow = 2
)

figure_prop_age_loess_bin

ggsave(here(plot_path, "figure_prop_age_loess_bin.png"),
  bg = "white", width = 14, height = 18
)
ggsave(here(paper_path, "Figure10.png"),
  bg = "white", width = 14, height = 18
)
```
...
rm(bm_aoi_toddlers_01inflation)
```
...
### Above/Below Chance Looking per Condition 

Fitting the main Bayesian hierarchical model with adjusted coding for condition to test for above or below chance performance in each condition (ignorance and knowledge) on first-trial proportion target looking during the anticipatory window.
...
# Adjust proportion looking to exit for chance level and respective condition to be set to 0
summarize_participant_test_first_trial <- summarize_participant_test_first_trial %>%
  mutate(
    prop_exit_adj = prop_exit - 0.5, # adjusting for chance
    condition_k = case_when(
      condition == "knowledge" ~ 0, # knowledge as reference condition
      condition == "ignorance" ~ 1), 
    condition_i = case_when(
      condition == "knowledge" ~ -1, # ignorance as reference condition
      condition == "ignorance" ~ 0)
  )

bm_aoi_toddlers_knowledge <- brm(
  prop_exit_adj ~ 1 + condition_k + age_mo_c + condition_k : age_mo_c + 
    (1 + condition_k + age_mo_c + condition_k : age_mo_c  | lab_id),
  family = gaussian,
  prior = priors,
  save_pars = save_pars(all = TRUE),
  filter(summarize_participant_test_first_trial, age_cohort == "toddlers"), 
...
summary(bm_aoi_toddlers_knowledge)
# prior_summary(bm_aoi_toddlers_knowledge)

saveRDS(bm_aoi_toddlers_knowledge,
        file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_toddlers_knowledge.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_knowledge <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_toddlers_knowledge.rds"))
```

Organize table in a tidy format

```{r}
bm_test_aoi_toddlers_knowledge_tidy_summary <- as_tibble(summary(bm_aoi_toddlers_knowledge)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_aoi_toddlers_knowledge_tidy_summary,
        file = here(RESULTS_FOLDER, "bm_test_aoi_toddlers_knowledge_model_summary.rds")
)
```

Compute Bayes Factor (bridge sampling approach) - propDLS different from chance in knowledge condition? 

```{r, eval = FIT_BAYES_MODELS}
bm_aoi_toddlers_knowledge_null <- update(bm_aoi_toddlers_knowledge, formula = ~ . - 1) # remove intercept
summary(bm_aoi_toddlers_knowledge_null)

saveRDS(bm_aoi_toddlers_knowledge_null,
        file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_toddlers_knowledge_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_knowledge_null <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_toddlers_knowledge_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_PTL_toddlers_knowledge <- brms::bayes_factor(
  bm_aoi_toddlers_knowledge, bm_aoi_toddlers_knowledge_null
)

saveRDS(test_m_comparison_PTL_toddlers_knowledge,
        file = here(RESULTS_FOLDER, "test_m_comparison_PTL_toddlers_knowledge.rds")
)
```
...
```{r}
rm(bm_aoi_toddlers_knowledge)
rm(bm_aoi_toddlers_knowledge_null)
```
...
bm_aoi_toddlers_ignorance <- brm(
  prop_exit_adj ~ 1 + condition_i + age_mo_c + condition_i : age_mo_c + 
    (1 + condition_i + age_mo_c + condition_i : age_mo_c | lab_id),
  family = gaussian,
  prior = priors,
  save_pars = save_pars(all = TRUE),
  filter(summarize_participant_test_first_trial, age_cohort == "toddlers"), 
...
summary(bm_aoi_toddlers_ignorance)
# prior_summary(bm_aoi_toddlers_ignorance)

saveRDS(bm_aoi_toddlers_ignorance,
        file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_toddlers_ignorance.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_ignorance <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_toddlers_ignorance.rds"))
```

Organize table in a tidy format

```{r}
bm_test_aoi_toddlers_ignorance_tidy_summary <- as_tibble(summary(bm_aoi_toddlers_ignorance)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_aoi_toddlers_ignorance_tidy_summary,
        file = here(RESULTS_FOLDER, "bm_test_aoi_toddlers_ignorance_model_summary.rds")
)
```

Compute Bayes Factor (bridge sampling approach) - propDLS different from chance in ignorance condition? 

```{r, eval = FIT_BAYES_MODELS}
bm_aoi_toddlers_ignorance_null <- update(bm_aoi_toddlers_ignorance, formula = ~ . - 1) # remove intercept
summary(bm_aoi_toddlers_ignorance_null)

saveRDS(bm_aoi_toddlers_ignorance_null,
        file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_toddlers_ignorance_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_ignorance_null <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_toddlers_ignorance_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_PTL_toddlers_ignorance <- brms::bayes_factor(
  bm_aoi_toddlers_ignorance, bm_aoi_toddlers_ignorance_null
)

saveRDS(test_m_comparison_PTL_toddlers_ignorance,
        file = here(RESULTS_FOLDER, "test_m_comparison_PTL_toddlers_ignorance.rds")
)
```
...
```{r}
rm(bm_aoi_toddlers_ignorance)
rm(bm_aoi_toddlers_ignorance_null)
```
...
bm_aoi_adults_knowledge <- brm(
  prop_exit_adj ~ 1 + condition_k + 
    (1 + condition_k | lab_id),
  family = gaussian,
  prior = priors,
  save_pars = save_pars(all = TRUE),
  filter(summarize_participant_test_first_trial, age_cohort == "adults"), 
...
summary(bm_aoi_adults_knowledge)
# prior_summary(bm_aoi_adults_knowledge)

saveRDS(bm_aoi_adults_knowledge,
        file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_adults_knowledge.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_knowledge <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_adults_knowledge.rds"))
```

Organize table in a tidy format

```{r}
bm_test_aoi_adults_knowledge_tidy_summary <- as_tibble(summary(bm_aoi_adults_knowledge)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_aoi_adults_knowledge_tidy_summary,
        file = here(RESULTS_FOLDER, "bm_test_aoi_adults_knowledge_model_summary.rds")
)
```
...
```{r, eval = FIT_BAYES_MODELS}
bm_aoi_adults_knowledge_null <- update(bm_aoi_adults_knowledge, formula = ~ . - 1) # remove intercept
summary(bm_aoi_adults_knowledge_null)

saveRDS(bm_aoi_adults_knowledge_null,
        file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_adults_knowledge_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_knowledge_null <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_adults_knowledge_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_PTL_adults_knowledge <- brms::bayes_factor(
  bm_aoi_adults_knowledge, bm_aoi_adults_knowledge_null
)

saveRDS(test_m_comparison_PTL_adults_knowledge,
        file = here(RESULTS_FOLDER, "test_m_comparison_PTL_adults_knowledge.rds")
)
```
...
```{r}
rm(bm_aoi_adults_knowledge)
rm(bm_aoi_adults_knowledge_null)
```
...
bm_aoi_adults_ignorance <- brm(
  prop_exit_adj ~ 1 + condition_i + 
    (1 + condition_i | lab_id),
  family = gaussian,
  prior = priors,
  save_pars = save_pars(all = TRUE),
  filter(summarize_participant_test_first_trial, age_cohort == "adults"), 
...
summary(bm_aoi_adults_ignorance)
# prior_summary(bm_aoi_adults_ignorance)

saveRDS(bm_aoi_adults_ignorance,
        file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_adults_ignorance.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_ignorance <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_adults_ignorance.rds"))
```

Organize table in a tidy format

```{r}
bm_test_aoi_adults_ignorance_tidy_summary <- as_tibble(summary(bm_aoi_adults_ignorance)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_aoi_adults_ignorance_tidy_summary,
        file = here(RESULTS_FOLDER, "bm_test_aoi_adults_ignorance_model_summary.rds")
)
```
...
```{r, eval = FIT_BAYES_MODELS}
bm_aoi_adults_ignorance_null <- update(bm_aoi_adults_ignorance, formula = ~ . - 1) # remove intercept
summary(bm_aoi_adults_ignorance_null)

saveRDS(bm_aoi_adults_ignorance_null,
        file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_adults_ignorance_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_ignorance_null <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_aoi_adults_ignorance_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_PTL_adults_ignorance <- brms::bayes_factor(
  bm_aoi_adults_ignorance, bm_aoi_adults_ignorance_null
)

saveRDS(test_m_comparison_PTL_adults_ignorance,
        file = here(RESULTS_FOLDER, "test_m_comparison_PTL_adults_ignorance.rds")
)
```
...
```{r}
rm(bm_aoi_adults_ignorance)
rm(bm_aoi_adults_ignorance_null)
```
...
rm(bm_test_first_look_toddlers_interaction_null)
```
...
Fitting the exploratory Bayesian hierarchical model testing above chance performance in each condition (ignorance and knowledge) on first-trial first looks during the anticipatory window.
...
test_first_look_first_trial <- d_anticipatory_test %>%
  ...
  ungroup() %>%
  # center age and set reference condition to 0
  mutate(
    age_mo_c = age_mo - mean(age_mo, na.rm = TRUE),
    condition_k = case_when(
      condition == "knowledge" ~ 0,
      condition == "ignorance" ~ 1
    ), 
    condition_i = case_when(
      condition == "knowledge" ~ -1,
      condition == "ignorance" ~ 0
    )
  )
```
...
bm_test_first_look_toddlers_knowledge <- brm(
  correct_first_look ~ 1 + condition_k + age_mo_c + condition_k : age_mo_c +
    (1 + condition_k + age_mo_c + condition_k : age_mo_c | lab_id),
  family = bernoulli(link = "logit"),
  prior = priors,
  save_pars = save_pars(all = TRUE),
...
summary(bm_test_first_look_toddlers_knowledge)
# prior_summary(bm_test_first_look_toddlers_knowledge)

saveRDS(bm_test_first_look_toddlers_knowledge,
  file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_toddlers_knowledge.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_first_look_toddlers_knowledge <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_toddlers_knowledge.rds"))
```

Organize table in a tidy format

```{r}
bm_test_fl_toddlers_knowledge_tidy_summary <- as_tibble(summary(bm_test_first_look_toddlers_knowledge)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_fl_toddlers_knowledge_tidy_summary,
  file = here(RESULTS_FOLDER, "bm_test_fl_toddlers_knowledge_model_summary.rds")
)
```

Compute Bayes Factor (bridge sampling approach) - First Looks different from chance in knowledge condition?

```{r, eval = FIT_BAYES_MODELS}
bm_test_first_look_toddlers_knowledge_null <- update(bm_test_first_look_toddlers_knowledge, formula = ~ . - 1)
summary(bm_test_first_look_toddlers_knowledge_null)

# cache
saveRDS(bm_test_first_look_toddlers_knowledge_null,
  file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_toddlers_knowledge_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_first_look_toddlers_knowledge_null <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_toddlers_knowledge_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_FL_toddlers_knowledge <- brms::bayes_factor(
  bm_test_first_look_toddlers_knowledge, bm_test_first_look_toddlers_knowledge_null
)

saveRDS(test_m_comparison_FL_toddlers_knowledge,
  file = here(RESULTS_FOLDER, "test_m_comparison_FL_toddlers_knowledge.rds")
)
```

```{r}
# remove Bayes model fits
rm(bm_test_first_look_toddlers_knowledge)
rm(bm_test_first_look_toddlers_knowledge_null)
```
...
bm_test_first_look_toddlers_ignorance <- brm(
  correct_first_look ~ 1 + condition_i + age_mo_c + condition_i : age_mo_c +
    (1 + condition_i + age_mo_c + condition_i : age_mo_c | lab_id),
  family = bernoulli(link = "logit"),
  prior = priors,
  save_pars = save_pars(all = TRUE),
...
summary(bm_test_first_look_toddlers_ignorance)
# prior_summary(bm_test_first_look_toddlers_ignorance)

saveRDS(bm_test_first_look_toddlers_ignorance,
  file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_toddlers_ignorance.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_first_look_toddlers_ignorance <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_toddlers_ignorance.rds"))
```

Organize table in a tidy format

```{r}
bm_test_fl_toddlers_ignorance_tidy_summary <- as_tibble(summary(bm_test_first_look_toddlers_ignorance)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_fl_toddlers_ignorance_tidy_summary,
  file = here(RESULTS_FOLDER, "bm_test_fl_toddlers_ignorance_model_summary.rds")
)
```

Compute Bayes Factor (bridge sampling approach) - First Looks different from chance in ignorance condition?

```{r, eval = FIT_BAYES_MODELS}
bm_test_first_look_toddlers_ignorance_null <- update(bm_test_first_look_toddlers_ignorance, formula = ~ . - 1)
summary(bm_test_first_look_toddlers_ignorance_null)

# cache
saveRDS(bm_test_first_look_toddlers_ignorance_null,
  file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_toddlers_ignorance_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_first_look_toddlers_ignorance_null <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_toddlers_ignorance_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_FL_toddlers_ignorance <- brms::bayes_factor(
  bm_test_first_look_toddlers_ignorance, bm_test_first_look_toddlers_ignorance_null
)

saveRDS(test_m_comparison_FL_toddlers_ignorance,
  file = here(RESULTS_FOLDER, "test_m_comparison_FL_toddlers_ignorance.rds")
)
```

```{r}
# remove Bayes model fits
rm(bm_test_first_look_toddlers_ignorance)
rm(bm_test_first_look_toddlers_ignorance_null)
```
...
bm_test_first_look_adults_knowledge <- brm(
  correct_first_look ~ 1 + condition_k + (1 + condition_k | lab_id),
  family = bernoulli(link = "logit"),
  prior = priors,
  save_pars = save_pars(all = TRUE),
  filter(test_first_look_first_trial, age_cohort == "adults"),
...
summary(bm_test_first_look_adults_knowledge)
# prior_summary(bm_test_first_look_adults_knowledge)

saveRDS(bm_test_first_look_adults_knowledge,
  file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_adults_knowledge.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_first_look_adults_knowledge <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_adults_knowledge.rds"))
```

Organize table in a tidy format

```{r}
bm_test_fl_adults_knowledge_tidy_summary <- as_tibble(summary(bm_test_first_look_adults_knowledge)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_fl_adults_knowledge_tidy_summary,
  file = here(RESULTS_FOLDER, "bm_test_fl_adults_knowledge_model_summary.rds")
)
```
...
```{r, eval = FIT_BAYES_MODELS}
bm_test_first_look_adults_knowledge_null <- update(bm_test_first_look_adults_knowledge, formula = ~ . - 1)
summary(bm_test_first_look_adults_knowledge_null)

# cache
saveRDS(bm_test_first_look_adults_knowledge_null,
  file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_adults_knowledge_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_first_look_adults_knowledge_null <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_adults_knowledge_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_FL_adults_knowledge <- brms::bayes_factor(
  bm_test_first_look_adults_knowledge, bm_test_first_look_adults_knowledge_null
)

saveRDS(test_m_comparison_FL_adults_knowledge,
  file = here(RESULTS_FOLDER, "test_m_comparison_FL_adults_knowledge.rds")
)
```

```{r}
# remove Bayes model fits
rm(bm_test_first_look_adults_knowledge)
rm(bm_test_first_look_adults_knowledge_null)
```

#### Adults Ignorance

```{r, eval = FIT_BAYES_MODELS}
bm_test_first_look_adults_ignorance <- brm(
  correct_first_look ~ 1 + condition_i + (1 + condition_i | lab_id),
  family = bernoulli(link = "logit"),
  prior = priors,
  save_pars = save_pars(all = TRUE),
  filter(test_first_look_first_trial, age_cohort == "adults"),
...
summary(bm_test_first_look_adults_ignorance)
# prior_summary(bm_test_first_look_adults_ignorance)

saveRDS(bm_test_first_look_adults_ignorance,
  file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_adults_ignorance.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_first_look_adults_ignorance <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_adults_ignorance.rds"))
```

Organize table in a tidy format

```{r}
bm_test_fl_adults_ignorance_tidy_summary <- as_tibble(summary(bm_test_first_look_adults_ignorance)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_fl_adults_ignorance_tidy_summary,
  file = here(RESULTS_FOLDER, "bm_test_fl_adults_ignorance_model_summary.rds")
)
```
...
```{r, eval = FIT_BAYES_MODELS}
bm_test_first_look_adults_ignorance_null <- update(bm_test_first_look_adults_ignorance, formula = ~ . - 1)
summary(bm_test_first_look_adults_ignorance_null)

# cache
saveRDS(bm_test_first_look_adults_ignorance_null,
  file = here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_adults_ignorance_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_first_look_adults_ignorance_null <- readRDS(here(RESULTS_FOLDER, "bayes_model_fits", "bm_test_first_look_adults_ignorance_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_FL_adults_ignorance <- brms::bayes_factor(
  bm_test_first_look_adults_ignorance, bm_test_first_look_adults_ignorance_null
)

saveRDS(test_m_comparison_FL_adults_ignorance,
  file = here(RESULTS_FOLDER, "test_m_comparison_FL_adults_ignorance.rds")
)
```

```{r}
# remove Bayes model fits
rm(bm_test_first_look_adults_ignorance)
rm(bm_test_first_look_adults_ignorance_null)
```
