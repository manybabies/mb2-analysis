...
overall_p_fam <- ggplot(filter(summarize_participant_familiarization, N_exit >= 5), 
...
```{r}
# plot average proportion looking
overall_p_test <- ggplot(summarize_participant_test_first_trial, 
...
```{r}
adults_prop <- ggplot(filter(summarize_participant_test_first_trial, N_exit >= 5 & age_cohort == "adults"), 
                      aes(x = condition, y = prop_exit, color = condition)) +
  # geom_violin()+
  # geom_boxplot()+
  geom_half_violin(data = filter(summarize_participant_test_first_trial, N_exit >= 5 & 
                                   condition == "ignorance" & age_cohort == "adults"), 
                   aes(fill = condition), side = "l", nudge = 0.3, width = 0.4, alpha = 0.2) +
  geom_half_violin(data = filter(summarize_participant_test_first_trial, N_exit >= 5 & 
                                   condition == "knowledge" & age_cohort == "adults"), 
                   aes(fill = condition), side = "r", nudge = 0.3, width = 0.4, alpha = 0.2) +
  geom_half_boxplot(data = filter(summarize_participant_test_first_trial, N_exit >= 5 & 
                                    condition == "ignorance" & age_cohort == "adults"), 
...
kids_prop <- ggplot(filter(summarize_participant_test_first_trial, N_exit >= 5 & age_cohort == "toddlers"), 
                    aes(x = condition, y = prop_exit, color = condition)) +
  # geom_violin()+
  # geom_boxplot()+
  geom_half_violin(data = filter(summarize_participant_test_first_trial, N_exit >= 5 & 
...
```{r}
kid_prop_by_age <- ggplot(filter(summarize_participant_test_first_trial, N_exit >= 5 & age_cohort == "toddlers"), 
...
bm_aoi_toddlers <- brm(prop_exit ~ 1 + condition_c + age_mo_c + condition_c * age_mo_c + 
...
bm_aoi_toddlers %>%
  ...
  theme_cowplot() +
  scale_x_discrete(limits = c("b_condition_c:age_mo_c", "b_age_mo_c", "b_condition_c"), 
...
rle_fam_data <- fam_data %>%
  ...
  group_by(age_cohort, age_mo, age_years_n, participant_lab_id, lab_id, 
...
ggplot(filter(d_anticipatory_fam, !is.na(first_look)), 
...
```{r}
overall_fl_fam <- ggplot(summarize_overall_familiarization_first_look, 
  ...
  geom_smooth(data = summarize_participant_familiarization_first_look, 
...
ggplot(filter(d_anticipatory_test, !is.na(first_look)), 
       aes(condition, fill = first_look)) +
  geom_bar(position = "fill") +
  facet_wrap(~age_cohort)

ggplot(summarize_overall_test_first_look, 
...
ggplot(filter(d_anticipatory_test, !is.na(first_look) & trial_num == 5), 
       aes(condition, fill = first_look)) +
  geom_bar(position = "fill") +
  facet_wrap(~age_cohort)

overall_fl_test <- ggplot(summarize_overall_test_first_look_first_trial, 
                          aes(condition, prop_target_first_look, color = condition)) +
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0) +
  geom_point(size = 5) +
  geom_hline(yintercept = 0.5, linetype = "dashed") +
...
# cache
saveRDS(bm_test_first_look_toddlers_intercept_effect, 
        file = here(RESULTS_FOLDER, "bm_test_first_look_toddlers_intercept_effect.rds"))
```
