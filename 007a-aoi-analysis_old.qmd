...
data_preprocessed_post_exclusions2 <- arrow::open_dataset(here(INTERMEDIATE_FOLDER, INTERMEDIATE_006_par), format = "parquet") %>% collect()
...
```{r}
#Is Rhat<1.1?
bm_fam_aoi_adults_tidy_summary$rhat
sum(bm_fam_aoi_adults_tidy_summary$rhat>=1.1)

#visual posterior predictive check
pp_check(bm_fam_aoi_adults) #one can clearly see that the model misses the zero-/one-inflation structure in the distribution
```
...
```{r}
bm_aoi_adults_tidy_summary <- as_tibble(summary(bm_aoi_adults)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_aoi_adults_tidy_summary,
  file = here(RESULTS_FOLDER, "bm_aoi_adults_model_summary.rds")
)
```
...
```{r}
#Is Rhat<1.1?
bm_aoi_adults_tidy_summary$rhat
sum(bm_aoi_adults_tidy_summary$rhat>=1.1)

#visual posterior predictive check
pp_check(bm_aoi_adults) #one can clearly see that the model misses the zero-/one-inflation structure in the distribution
```
...
```{r}
bm_fam_first_look_toddlers_tidy_summary <- as_tibble(summary(bm_fam_first_look_toddlers)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_fam_first_look_toddlers_tidy_summary,
  file = here(RESULTS_FOLDER, "bm_fam_first_look_toddlers_model_summary.rds")
)
```
...
```{r}
#Is Rhat<1.1?
bm_fam_first_look_toddlers_tidy_summary$rhat
sum(bm_fam_first_look_toddlers_tidy_summary$rhat>=1.1)

#visual posterior predictive check
pp_check(bm_fam_first_look_toddlers)
```
