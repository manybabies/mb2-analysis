...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_second_trial_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_second_trial_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_second_trial <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_second_trial.rds"))
```
...
Compute the Bayes factor using bridge sampling - condition effect
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_second_trial_condition_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_second_trial_condition_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_second_trial_condition <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_second_trial_condition.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_second_trial_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_second_trial_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_second_trial <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_second_trial.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_second_trial_condition_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_second_trial_condition_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_second_trial_condition <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_second_trial_condition.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_correct_final_fam_trial_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_correct_final_fam_trial_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_correct_final_fam_trial <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_correct_final_fam_trial.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_correct_final_fam_trial_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_correct_final_fam_trial_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_correct_final_fam_trial <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_correct_final_fam_trial.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4 <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_sufficient_fam_prop_looking_3and4_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_sufficient_fam_prop_looking_3and4_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4 <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4.rds"))
```
...
saveRDS(h_test_adults_shifts_al_interaction, 
        file = here(RESULTS_FOLDER, "h_test_m_comparison_adults_shifts_al_interaction.rds"))
```
...
saveRDS(h_test_adults_shifts_al_num_shifts, 
        file = here(RESULTS_FOLDER, "h_test_m_comparison_adults_shifts_al_num_shifts.rds"))
```
