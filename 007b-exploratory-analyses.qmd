...
# parameter for controlling whether to compute Bayes factors (computationally expensive)
...
#loess fit
kid_prop_by_age_loess_both <- ggplot(filter(summarize_participant_test_both_trials, 
                                      N_exit >= 5 & age_cohort == "toddlers"), 
                               aes(x = age_mo, y = prop_exit, color = condition)) +
  geom_hline(yintercept = 0.5, linetype = "dashed") +
  geom_point(alpha = 0.4) +
  ...
  scale_color_brewer(palette = "Set1") +
  scale_fill_brewer(palette = "Set1")
kid_prop_by_age_loess_both
ggsave(here(plot_path, "kids_proportion_both_trials_target_exit_looking_by_age_loess.png"), 
       bg = "white", width = 9, height = 6)
```
...
#Note: the Bayes factors we obtained for this model were more variable,
#so we increased the number of iterations in an attempt to obtain more stable
# Bayes factor estimates (using bridge sampling)
bm_aoi_toddlers_both <- brm(
...
bm_aoi_toddlers_both <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_both.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_both_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_both_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_comparison_aoi_toddlers_both <- readRDS(here(RESULTS_FOLDER, "test_comparison_aoi_toddlers_both.rds"))
```

Alternative bayes factor computation (for comparison, due to the above-mentioned challenges with bridge sampling)
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_both <- readRDS(here(model_fits_dir, "bm_aoi_adults_both.rds"))
```
...
saveRDS(bm_aoi_adults_both_null, 
        file = here(model_fits_dir, "bm_aoi_adults_both_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_both_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_both_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_comparison_aoi_adults_both <- readRDS(here(RESULTS_FOLDER, "test_comparison_aoi_adults_both.rds"))
```
...
## 1.b. whether effects are magnified or diminished on the second trial (order main effect), and 1.c. whether there is evidence of “spillover” - dependency (condition x order interaction effect)
...
bm_aoi_toddlers_both_order <- brm_default(
  prop_exit ~ 1 + condition_c + test_trial_num + condition_c : test_trial_num + 
    ...
    (1 + condition_c | participant_lab_id), # to be consistent, we decided to include random slope for condition within participant (if model converges) 
  family = gaussian,
  prior = priors_default,
  data = filter(summarize_participant_test_both_trials, age_cohort == "toddlers")
)
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_both_order <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_both_order.rds"))
```
...
Compute the Bayes factor using bridge sampling - Trial Number Effect
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_both_order_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_both_order_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_comparison_aoi_toddlers_both_order <- readRDS(here(RESULTS_FOLDER, "test_comparison_aoi_toddlers_both_order.rds"))
```

Alternative bayes factor computation for Test Trial Number Effect (due to the challenges with bridge sampling)

```{r}
h_test_aoi_toddlers_both_order <- hypothesis(bm_aoi_toddlers_both_order, "test_trial_num = 0", class = "b")
h_test_aoi_toddlers_both_order
plot(h_test_aoi_toddlers_both_order)
# evidence in favor of condition being different from zero
1 / h_test_aoi_toddlers_both_order$hypothesis$Evid.Ratio
saveRDS(h_test_aoi_toddlers_both_order, 
        file = here(RESULTS_FOLDER, "h_test_aoi_toddlers_both_order.rds"))
```
...
saveRDS(bm_aoi_toddlers_both_order_interaction_null, 
        file = here(model_fits_dir, "bm_aoi_toddlers_both_order_interaction_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_both_order_interaction_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_both_order_interaction_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_PTL_toddlers_both_order_interaction <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_PTL_toddlers_both_order_interaction.rds"))
```

Alternative bayes factor computation for Condition*Test Trial Number Interaction Effect (due to the challenges with bridge sampling)

```{r}
h_test_aoi_toddlers_both_order_interaction <- hypothesis(bm_aoi_toddlers_both_order, "condition_c:test_trial_num = 0", class = "b")
h_test_aoi_toddlers_both_order_interaction
plot(h_test_aoi_toddlers_both_order_interaction)
# evidence in favor of condition being different from zero
1 / h_test_aoi_toddlers_both_order_interaction$hypothesis$Evid.Ratio
saveRDS(h_test_aoi_toddlers_both_order_interaction, 
        file = here(RESULTS_FOLDER, "h_test_aoi_toddlers_both_order_interaction.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_both_order <- readRDS(here(model_fits_dir, "bm_aoi_adults_both_order.rds"))
```
...
Compute the Bayes factor using bridge sampling - Test Trial Number Effect
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_both_order_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_both_order_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_comparison_aoi_adults_both_order <- readRDS(here(RESULTS_FOLDER, "test_comparison_aoi_adults_both_order.rds"))
```

Alternative bayes factor computation for Test Trial Number Effect (due to the challenges with bridge sampling)

```{r}
h_test_aoi_adults_both_order <- hypothesis(bm_aoi_adults_both_order, "test_trial_num = 0", class = "b")
h_test_aoi_adults_both_order
plot(h_test_aoi_adults_both_order)
# evidence in favor of condition being different from zero
1 / h_test_aoi_adults_both_order$hypothesis$Evid.Ratio
saveRDS(h_test_aoi_adults_both_order, 
        file = here(RESULTS_FOLDER, "h_test_aoi_adults_both_order.rds"))
```
...
saveRDS(bm_aoi_adults_both_order_interaction_null, 
        file = here(model_fits_dir, "bm_aoi_adults_both_order_interaction_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_both_order_interaction_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_both_order_interaction_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_PTL_adults_both_order_interaction <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_PTL_adults_both_order_interaction.rds"))
```

Alternative bayes factor computation for Condition * Test Trial Number Interaction Effect (due to the challenges with bridge sampling)

```{r}
h_test_aoi_adults_both_order_interaction <- hypothesis(bm_aoi_adults_both_order, "condition_c:test_trial_num = 0", class = "b")
h_test_aoi_adults_both_order_interaction
plot(h_test_aoi_adults_both_order_interaction)
# evidence in favor of condition being different from zero
1 / h_test_aoi_adults_both_order_interaction$hypothesis$Evid.Ratio
saveRDS(h_test_aoi_adults_both_order_interaction, 
        file = here(RESULTS_FOLDER, "h_test_aoi_adults_both_order_interaction.rds"))
```
...
rm(bm_aoi_adults_both_order_interaction_null)
gc()
```

### 1.d. whether there is evidence of “spillover” - dependency in anticipation on the second trial depending on what the first trial is (condition x first_trial interaction effect)
...
bm_aoi_toddlers_second_trial <- brm_default(
  second_trial_prop_exit ~ 1 + condition_c + first_trial_prop_exit_c + condition_c : first_trial_prop_exit_c +
    (1 + condition_c + first_trial_prop_exit_c + condition_c : first_trial_prop_exit_c | lab_id),
  family = gaussian,
  prior = priors_default,
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_second_trial <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_second_trial.rds"))
```
...
Compute the Bayes factor using bridge sampling - interaction effect

```{r, eval = FIT_BAYES_MODELS}
bm_aoi_toddlers_second_trial_interaction_null <- update(bm_aoi_toddlers_second_trial, 
                                            formula = ~ . - condition_c : first_trial_prop_exit_c,
                                            cores = n_cores)
summary(bm_aoi_toddlers_second_trial_interaction_null)

saveRDS(bm_aoi_toddlers_second_trial_interaction_null, 
        file = here(model_fits_dir, "bm_aoi_toddlers_second_trial_interaction_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_second_trial_interaction_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_second_trial_interaction_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_toddlers_second_trial_interaction <- brms::bayes_factor(bm_aoi_toddlers_second_trial, 
                                                                  bm_aoi_toddlers_second_trial_interaction_null)

saveRDS(test_m_comparison_aoi_toddlers_second_trial_interaction, 
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_second_trial_interaction.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_second_trial_interaction <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_second_trial_interaction.rds"))
```

Compute the Bayes factor using bridge sampling - first_trial_prop_exit effect

```{r, eval = FIT_BAYES_MODELS}
bm_aoi_toddlers_second_trial_first_trial_null <- update(bm_aoi_toddlers_second_trial, 
                                            formula = ~ . - first_trial_prop_exit_c,
                                            cores = n_cores)
summary(bm_aoi_toddlers_second_trial_first_trial_null)

saveRDS(bm_aoi_toddlers_second_trial_first_trial_null, 
        file = here(model_fits_dir, "bm_aoi_toddlers_second_trial_first_trial_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_second_trial_first_trial_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_second_trial_first_trial_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_toddlers_second_trial_first_trial <- brms::bayes_factor(bm_aoi_toddlers_second_trial, 
                                                                  bm_aoi_toddlers_second_trial_first_trial_null)

saveRDS(test_m_comparison_aoi_toddlers_second_trial_first_trial, 
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_second_trial_first_trial.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_second_trial_first_trial <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_second_trial_first_trial.rds"))
```
...
```{r}
# remove Bayes model fits
rm(bm_aoi_toddlers_second_trial)
rm(bm_aoi_toddlers_second_trial_interaction_null)
rm(bm_aoi_toddlers_second_trial_first_trial_null)
gc()
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_second_trial <- readRDS(here(model_fits_dir, "bm_aoi_adults_second_trial.rds"))
```
...
```{r, eval = FIT_BAYES_MODELS}
bm_aoi_adults_second_trial_interaction_null <- update(bm_aoi_adults_second_trial, 
                                          formula = ~ . - condition_c : first_trial_prop_exit_c,
                                          cores = n_cores)
summary(bm_aoi_adults_second_trial_interaction_null)

saveRDS(bm_aoi_adults_second_trial_interaction_null, 
        file = here(model_fits_dir, "bm_aoi_adults_second_trial_interaction_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_second_trial_interaction_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_second_trial_interaction_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_adults_second_trial_interaction <- brms::bayes_factor(bm_aoi_adults_second_trial, 
                                                                bm_aoi_adults_second_trial_interaction_null)

saveRDS(test_m_comparison_aoi_adults_second_trial_interaction, 
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_second_trial_interaction.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_second_trial_interaction <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_second_trial_interaction.rds"))
```

Compute the Bayes factor using bridge sampling - first_trial_prop_exit effect

```{r, eval = FIT_BAYES_MODELS}
bm_aoi_adults_second_trial_first_trial_null <- update(bm_aoi_adults_second_trial, 
                                          formula = ~ . - first_trial_prop_exit_c,
                                          cores = n_cores)
summary(bm_aoi_adults_second_trial_first_trial_null)

saveRDS(bm_aoi_adults_second_trial_first_trial_null, 
        file = here(model_fits_dir, "bm_aoi_adults_second_trial_first_trial_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_second_trial_first_trial_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_second_trial_first_trial_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_adults_second_trial_first_trial <- brms::bayes_factor(bm_aoi_adults_second_trial, 
                                                                bm_aoi_adults_second_trial_first_trial_null)

saveRDS(test_m_comparison_aoi_adults_second_trial_first_trial, 
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_second_trial_first_trial.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_second_trial_first_trial <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_second_trial_first_trial.rds"))
```
...
```{r}
# remove Bayes model fits
rm(bm_aoi_adults_second_trial)
rm(bm_aoi_adults_second_trial_interaction_null)
rm(bm_aoi_adults_second_trial_first_trial_null)
gc()
```
...
Fitting the main Bayesian hierarchical model testing the effect of condition (ignorance vs. knowledge) on first-trial proportion target looking during the anticipatory window and its interaction with whether or not participants anticipated correctly during the last familiarization trial (trial 4, first look to target).
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_correct_final_fam_trial <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_correct_final_fam_trial.rds"))
```
...
# cache
saveRDS(bm_aoi_toddlers_correct_final_fam_trial_condition_correct_first_look_final_fam_trial_interaction, 
        file = here(RESULTS_FOLDER, "bm_aoi_toddlers_correct_final_fam_trial_condition_correct_first_look_final_fam_trial_interaction.rds"))
```

Compute the Bayes factor using bridge sampling - interaction effect

```{r, eval = FIT_BAYES_MODELS}
bm_aoi_toddlers_correct_final_fam_trial_interaction_null <- update(bm_aoi_toddlers_correct_final_fam_trial, 
                                                       formula = ~ . - condition_c : correct_first_look_final_fam_trial_c,
                                                       cores = n_cores)
summary(bm_aoi_toddlers_correct_final_fam_trial_interaction_null)

saveRDS(bm_aoi_toddlers_correct_final_fam_trial_interaction_null, 
        file = here(model_fits_dir, "bm_aoi_toddlers_correct_final_fam_trial_interaction_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_correct_final_fam_trial_interaction_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_correct_final_fam_trial_interaction_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_toddlers_correct_final_fam_trial_interaction <- brms::bayes_factor(bm_aoi_toddlers_correct_final_fam_trial, bm_aoi_toddlers_correct_final_fam_trial_interaction_null)

saveRDS(test_m_comparison_aoi_toddlers_correct_final_fam_trial_interaction, 
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_correct_final_fam_trial_interaction.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_correct_final_fam_trial_interaction <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_correct_final_fam_trial_interaction.rds"))
```

Compute the Bayes factor using bridge sampling - correct_first_look_final_fam_trial effect

```{r, eval = FIT_BAYES_MODELS}
bm_aoi_toddlers_correct_final_fam_trial_FL_fam_null <- update(bm_aoi_toddlers_correct_final_fam_trial, 
                                                       formula = ~ . - correct_first_look_final_fam_trial_c,
                                                       cores = n_cores)
summary(bm_aoi_toddlers_correct_final_fam_trial_FL_fam_null)

saveRDS(bm_aoi_toddlers_correct_final_fam_trial_FL_fam_null, 
        file = here(model_fits_dir, "bm_aoi_toddlers_correct_final_fam_trial_FL_fam_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_correct_final_fam_trial_FL_fam_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_correct_final_fam_trial_FL_fam_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_toddlers_correct_final_fam_trial_FL_fam <- brms::bayes_factor(bm_aoi_toddlers_correct_final_fam_trial, bm_aoi_toddlers_correct_final_fam_trial_FL_fam_null)

saveRDS(test_m_comparison_aoi_toddlers_correct_final_fam_trial_FL_fam, 
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_correct_final_fam_trial_FL_fam.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_correct_final_fam_trial_FL_fam <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_correct_final_fam_trial_FL_fam.rds"))
```
...
rm(bm_aoi_toddlers_correct_final_fam_trial_interaction_null)
rm(bm_aoi_toddlers_correct_final_fam_trial_FL_fam_null)
gc()
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_correct_final_fam_trial <- readRDS(here(model_fits_dir, "bm_aoi_adults_correct_final_fam_trial.rds"))
```
...
```{r, eval = FIT_BAYES_MODELS}
bm_aoi_adults_correct_final_fam_trial_interaction_null <- update(bm_aoi_adults_correct_final_fam_trial, 
                                                     formula = ~ . - condition_c : correct_first_look_final_fam_trial_c,
                                                     cores = n_cores)
summary(bm_aoi_adults_correct_final_fam_trial_interaction_null)

saveRDS(bm_aoi_adults_correct_final_fam_trial_interaction_null, 
        file = here(model_fits_dir, "bm_aoi_adults_correct_final_fam_trial_interaction_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_correct_final_fam_trial_interaction_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_correct_final_fam_trial_interaction_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_adults_correct_final_fam_trial_interaction <- brms::bayes_factor(bm_aoi_adults_correct_final_fam_trial, bm_aoi_adults_correct_final_fam_trial_interaction_null)

saveRDS(test_m_comparison_aoi_adults_correct_final_fam_trial_interaction, 
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_correct_final_fam_trial_interaction.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_correct_final_fam_trial_interaction <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_correct_final_fam_trial_interaction.rds"))
```

Compute the Bayes factor using bridge sampling - correct_first_look_final_fam_trial effect

```{r, eval = FIT_BAYES_MODELS}
bm_aoi_adults_correct_final_fam_trial_FL_fam_null <- update(bm_aoi_adults_correct_final_fam_trial, 
                                                     formula = ~ . - correct_first_look_final_fam_trial_c,
                                                     cores = n_cores)
summary(bm_aoi_adults_correct_final_fam_trial_FL_fam_null)

saveRDS(bm_aoi_adults_correct_final_fam_trial_FL_fam_null, 
        file = here(model_fits_dir, "bm_aoi_adults_correct_final_fam_trial_FL_fam_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_correct_final_fam_trial_FL_fam_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_correct_final_fam_trial_FL_fam_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_adults_correct_final_fam_trial_FL_fam <- brms::bayes_factor(bm_aoi_adults_correct_final_fam_trial, bm_aoi_adults_correct_final_fam_trial_FL_fam_null)

saveRDS(test_m_comparison_aoi_adults_correct_final_fam_trial_FL_fam, 
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_correct_final_fam_trial_FL_fam.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_correct_final_fam_trial_FL_fam <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_correct_final_fam_trial_FL_fam.rds"))
```
Remove the Bayes models from the environment (because the model objects are quite large).
...
rm(bm_aoi_adults_correct_final_fam_trial_interaction_null)
rm(bm_aoi_adults_correct_final_fam_trial_FL_fam_null)
gc()
```
...
Fitting the main Bayesian hierarchical model testing the effect of condition (ignorance vs. knowledge) on first-trial proportion target looking during the anticipatory window and its interaction with whether or not participants fixated the target more than half of the time during all familiarization trials.
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_sufficient_fam_prop_looking_3and4 <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_sufficient_fam_prop_looking_3and4.rds"))
```
...
```{r, eval = FIT_BAYES_MODELS}
bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction_null <- update(bm_aoi_toddlers_sufficient_fam_prop_looking_3and4, 
                                                           formula = ~ . - condition_c : successful_fam_anticipation_c,
                                                           cores = n_cores)
summary(bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction_null)

saveRDS(bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction_null, 
        file = here(model_fits_dir, "bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction <-
  brms::bayes_factor(bm_aoi_toddlers_sufficient_fam_prop_looking_3and4, bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction_null)

saveRDS(test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction, 
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction.rds"))
```

Compute the Bayes factor using bridge sampling - successful_fam_anticipation effect

```{r, eval = FIT_BAYES_MODELS}
bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam_null <- update(bm_aoi_toddlers_sufficient_fam_prop_looking_3and4, 
                                                           formula = ~ . - successful_fam_anticipation_c,
                                                           cores = n_cores)
summary(bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam_null)

saveRDS(bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam_null, 
        file = here(model_fits_dir, "bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam <-
  brms::bayes_factor(bm_aoi_toddlers_sufficient_fam_prop_looking_3and4, bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam_null)

saveRDS(test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam, 
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam.rds"))
```
...
rm(bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_interaction_null)
rm(bm_aoi_toddlers_sufficient_fam_prop_looking_3and4_fam_null)
gc()
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_sufficient_fam_prop_looking_3and4 <- readRDS(here(model_fits_dir, "bm_aoi_adults_sufficient_fam_prop_looking_3and4.rds"))
```
...
```{r, eval = FIT_BAYES_MODELS}
bm_aoi_adults_sufficient_fam_prop_looking_3and4_interaction_null <- update(bm_aoi_adults_sufficient_fam_prop_looking_3and4, 
                                                         formula = ~ . - condition_c : successful_fam_anticipation_c,
                                                         cores = n_cores)
summary(bm_aoi_adults_sufficient_fam_prop_looking_3and4_interaction_null)

saveRDS(bm_aoi_adults_sufficient_fam_prop_looking_3and4_interaction_null, 
        file = here(model_fits_dir, "bm_aoi_adults_sufficient_fam_prop_looking_3and4_interaction_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_sufficient_fam_prop_looking_3and4_interaction_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_sufficient_fam_prop_looking_3and4_interaction_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4_interaction <- brms::bayes_factor(bm_aoi_adults_sufficient_fam_prop_looking_3and4, bm_aoi_adults_sufficient_fam_prop_looking_3and4_interaction_null)

saveRDS(test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4_interaction,
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4_interaction.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4_interaction <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4_interaction.rds"))
```

Compute the Bayes factor using bridge sampling - successful_fam_anticipation effect

```{r, eval = FIT_BAYES_MODELS}
bm_aoi_adults_sufficient_fam_prop_looking_3and4_fam_null <- update(bm_aoi_adults_sufficient_fam_prop_looking_3and4, 
                                                         formula = ~ . - successful_fam_anticipation_c,
                                                         cores = n_cores)
summary(bm_aoi_adults_sufficient_fam_prop_looking_3and4_fam_null)

saveRDS(bm_aoi_adults_sufficient_fam_prop_looking_3and4_fam_null, 
        file = here(model_fits_dir, "bm_aoi_adults_sufficient_fam_prop_looking_3and4_fam_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_sufficient_fam_prop_looking_3and4_fam_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_sufficient_fam_prop_looking_3and4_fam_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4_fam <- brms::bayes_factor(bm_aoi_adults_sufficient_fam_prop_looking_3and4, bm_aoi_adults_sufficient_fam_prop_looking_3and4_fam_null)

saveRDS(test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4_fam,
        file = here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4_fam.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4_fam <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_sufficient_fam_prop_looking_3and4_fam.rds"))
```
...
rm(bm_aoi_adults_sufficient_fam_prop_looking_3and4_interaction_null)
rm(bm_aoi_adults_sufficient_fam_prop_looking_3and4_fam_null)
gc()
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_sufficient_fam_prop_looking <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_sufficient_fam_prop_looking.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_sufficient_fam_prop_looking_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_sufficient_fam_prop_looking_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_sufficient_fam_prop_looking.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_sufficient_fam_prop_looking <- readRDS(here(model_fits_dir, "bm_aoi_adults_sufficient_fam_prop_looking.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_sufficient_fam_prop_looking_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_sufficient_fam_prop_looking_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_sufficient_fam_prop_looking <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_sufficient_fam_prop_looking.rds"))
```
...
```{r}
correlation_plot_fam_test <- ggplot(summarize_participant_test_first_trial_all_fam_trials,
...
correlation_fam_test_overall <- summarize_participant_test_first_trial_all_fam_trials %>%
...
correlation_fam_test_by_age <- summarize_participant_test_first_trial_all_fam_trials %>%
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_method <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_method.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_toddlers_method_null <- readRDS(here(model_fits_dir, "bm_aoi_toddlers_method_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_toddlers_method <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_toddlers_method.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_method <- readRDS(here(model_fits_dir, "bm_aoi_adults_method.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_aoi_adults_method_null <- readRDS(here(model_fits_dir, "bm_aoi_adults_method_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_aoi_adults_method <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_aoi_adults_method.rds"))
```
...
#loess fit
kid_prop_by_age_box_loess <- ggplot(filter(summarize_participant_test_first_trial, N_exit >= 5 & age_cohort == "toddlers"), 
                              aes(x = age_mo, y = prop_box, color = condition)) +
  geom_hline(yintercept = 0.5, linetype = "dashed") +
  geom_point(alpha = 0.4) +
  geom_smooth(method = "loess") +
  ...
  scale_color_brewer(palette = "Set1") +
  scale_fill_brewer(palette = "Set1")
kid_prop_by_age_box_loess
ggsave(here(plot_path, "kids_proportion_first_trial_target_box_looking_by_age_loess.png"), 
       bg = "white", width = 9, height = 6)
```
...
saveRDS(bm_box_toddlers,
  file = here(model_fits_dir, "bm_box_toddlers.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_box_toddlers <- readRDS(here(model_fits_dir, "bm_box_toddlers.rds"))
```
...
saveRDS(bm_box_toddlers_condition_null,
  file = here(model_fits_dir, "bm_box_toddlers_condition_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_box_toddlers_condition_null <- readRDS(here(model_fits_dir, "bm_box_toddlers_condition_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_box_m_comparison_PTL_toddlers <- readRDS(here(RESULTS_FOLDER, "test_box_m_comparison_PTL_toddlers.rds"))
```
...
saveRDS(bm_box_toddlers_interaction_null,
  file = here(model_fits_dir, "bm_box_toddlers_interaction_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_box_toddlers_interaction_null <- readRDS(here(model_fits_dir, "bm_box_toddlers_interaction_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_box_m_comparison_PTL_toddlers_interaction <- readRDS(here(RESULTS_FOLDER, "test_box_m_comparison_PTL_toddlers_interaction.rds"))
```
...
saveRDS(bm_box_adults,
  file = here(model_fits_dir, "bm_box_adults.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_box_adults <- readRDS(here(model_fits_dir, "bm_box_adults.rds"))
```
...
saveRDS(bm_box_adults_condition_null,
  file = here(model_fits_dir, "bm_box_adults_condition_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_box_adults_condition_null <- readRDS(here(model_fits_dir, "bm_box_adults_condition_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_box_m_comparison_PTL_adults <- readRDS(here(RESULTS_FOLDER, "test_box_m_comparison_PTL_adults.rds"))
```
...
priors_shifts <- c( set_prior( "normal(1, 0.5)", class = "Intercept"),
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_shifts_age_cohort <- readRDS(here(model_fits_dir, "bm_test_shifts_age_cohort.rds"))
```
...
```{r}
#Is Rhat<1.1?
bm_test_shifts_age_cohort$rhat
sum(bm_test_shifts_age_cohort$rhat>=1.1)

#visual posterior predictive check
pp_check(bm_test_shifts_age_cohort) 
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_shifts_age_cohort_null <- readRDS(here(model_fits_dir, "bm_test_shifts_age_cohort_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_shifts_age_cohort <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_shifts_age_cohort.rds"))
```

Alternative bayes factor computation for interaction effect (due to challenges with bridge sampling)

```{r}
h_test_shifts_age_cohort_interaction <- hypothesis(bm_test_shifts_age_cohort, "condition_c:age_cohort_c = 0", class = "b")
h_test_shifts_age_cohort_interaction
#plot(h_test_shifts_age_cohort_interaction)
# evidence in favor of interaction being different from zero
1 / h_test_shifts_age_cohort_interaction$hypothesis$Evid.Ratio
saveRDS(h_test_shifts_age_cohort_interaction, 
        file = here(RESULTS_FOLDER, "h_test_shifts_age_cohort_interaction.rds"))
```

Bayes factor for Age effect

```{r, eval = FIT_BAYES_MODELS}
bm_test_shifts_age_cohort_age_null <- update(bm_test_shifts_age_cohort, 
                                         formula = ~ . - age_cohort_c,
                                         cores = n_cores)
summary(bm_test_shifts_age_cohort_age_null)

saveRDS(bm_test_shifts_age_cohort_age_null, 
        file = here(model_fits_dir, "bm_test_shifts_age_cohort_age_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_shifts_age_cohort_age_null <- readRDS(here(model_fits_dir, "bm_test_shifts_age_cohort_age_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_shifts_age_cohort_age <- brms::bayes_factor(bm_test_shifts_age_cohort, 
                                                          bm_test_shifts_age_cohort_age_null)

saveRDS(test_m_comparison_shifts_age_cohort_age, 
        file = here(RESULTS_FOLDER, "test_m_comparison_shifts_age_cohort_age.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_shifts_age_cohort_age <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_shifts_age_cohort_age.rds"))
```

Alternative bayes factor computation for age cohort effect (due to challenges with bridge sampling)

```{r}
h_test_shifts_age_cohort_age <- hypothesis(bm_test_shifts_age_cohort, "age_cohort_c = 0", class = "b")
h_test_shifts_age_cohort_interaction
#plot(h_test_shifts_age_cohort_age)
# evidence in favor of age cohort being different from zero
1 / h_test_shifts_age_cohort_age$hypothesis$Evid.Ratio
saveRDS(h_test_shifts_age_cohort_age, 
        file = here(RESULTS_FOLDER, "h_test_shifts_age_cohort_age.rds"))
```
...
rm(bm_test_shifts_age_cohort_age_null)
gc()
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_shifts_toddlers <- readRDS(here(model_fits_dir, "bm_test_shifts_toddlers.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_shifts_toddlers_null <- readRDS(here(model_fits_dir, "bm_test_shifts_toddlers_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_shifts_toddlers <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_shifts_toddlers.rds"))
```

Alternative bayes factor computation for condition effect (due to challenges with bridge sampling)

```{r}
h_test_shifts_toddlers_condition <- hypothesis(bm_test_shifts_toddlers, "condition_c = 0", class = "b")
h_test_shifts_toddlers_condition
#plot(h_test_shifts_toddlers_condition)
# evidence in favor of age cohort being different from zero
1 / h_test_shifts_toddlers_condition$hypothesis$Evid.Ratio
saveRDS(h_test_shifts_toddlers_condition, 
        file = here(RESULTS_FOLDER, "h_test_shifts_toddlers_condition.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_shifts_adults <- readRDS(here(model_fits_dir, "bm_test_shifts_adults.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_shifts_adults_null <- readRDS(here(model_fits_dir, "bm_test_shifts_adults_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_shifts_adults <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_shifts_adults.rds"))
```

Alternative bayes factor computation for condition effect (due to challenges with bridge sampling)

```{r}
h_test_shifts_adults_condition <- hypothesis(bm_test_shifts_adults, "condition_c = 0", class = "b")
h_test_shifts_adults_condition
#plot(h_test_shifts_adults_condition)
# evidence in favor of age cohort being different from zero
1 / h_test_shifts_adults_condition$hypothesis$Evid.Ratio
saveRDS(h_test_shifts_adults_condition, 
        file = here(RESULTS_FOLDER, "h_test_shifts_adults_condition"))
```
...
#### Toddlers
```{r, eval = FIT_BAYES_MODELS}
bm_test_toddlers_shifts_al <- brm(
...
saveRDS(bm_test_toddlers_shifts_al, 
        file = here(model_fits_dir, "bm_test_toddlers_shifts_al.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_toddlers_shifts_al <- readRDS(here(model_fits_dir, "bm_test_toddlers_shifts_al.rds"))
```

Organize table in a tidy format

```{r}
bm_test_toddlers_shifts_al_tidy_summary <- as_tibble(summary(bm_test_toddlers_shifts_al)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_toddlers_shifts_al_tidy_summary,
  file = here(RESULTS_FOLDER, "bm_test_toddlers_shifts_al_model_summary.rds")
)
```
...
Bridge sampling Bayes factor approach: Condition Number of shifts Interaction Effect
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_toddlers_shifts_al_null <- readRDS(here(model_fits_dir, "bm_test_toddlers_shifts_al_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_toddlers_shifts_al <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_toddlers_shifts_al.rds"))
```

Bridge sampling Bayes factor approach: Number of shifts Effect

```{r, eval = FIT_BAYES_MODELS}
bm_test_toddlers_shifts_al_num_shifts_null <- update(bm_test_toddlers_shifts_al, 
                                          formula = ~ . - num_shifts,
                                          cores = n_cores)
summary(bm_test_toddlers_shifts_al_num_shifts_null)

saveRDS(bm_test_toddlers_shifts_al_num_shifts_null, 
        file = here(model_fits_dir, "bm_test_toddlers_shifts_al_num_shifts_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_toddlers_shifts_al_num_shifts_null <- readRDS(here(model_fits_dir, "bm_test_toddlers_shifts_al_num_shifts_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_toddlers_shifts_al_num_shifts <- brms::bayes_factor(bm_test_toddlers_shifts_al, 
                                                           bm_test_toddlers_shifts_al_num_shifts_null)

saveRDS(test_m_comparison_toddlers_shifts_al_num_shifts, 
        file = here(RESULTS_FOLDER, "test_m_comparison_toddlers_shifts_al_num_shifts.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_toddlers_shifts_al_num_shifts <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_toddlers_shifts_al_num_shifts.rds"))
```

Alternative bayes factor computation for interaction effect (due to challenges with bridge sampling)

```{r}
h_test_toddlers_shifts_al_interaction <- hypothesis(bm_test_toddlers_shifts_al, "condition_c:num_shifts = 0", class = "b")
h_test_toddlers_shifts_al_interaction
#plot(h_test_toddlers_shifts_al_interaction)
# evidence in favor of age cohort being different from zero
1 / h_test_toddlers_shifts_al_interaction$hypothesis$Evid.Ratio
saveRDS(h_test_toddlers_shifts_al_interaction, 
        file = here(RESULTS_FOLDER, "h_test_toddlers_shifts_al_interaction"))
```

Alternative bayes factor computation for number of shifts effect (due to challenges with bridge sampling)

```{r}
h_test_toddlers_shifts_al_num_shifts <- hypothesis(bm_test_toddlers_shifts_al, "num_shifts = 0", class = "b")
h_test_toddlers_shifts_al_num_shifts
#plot(h_test_toddlers_shifts_al_num_shifts)
# evidence in favor of age cohort being different from zero
1 / h_test_toddlers_shifts_al_num_shifts$hypothesis$Evid.Ratio
saveRDS(h_test_toddlers_shifts_al_num_shifts, 
        file = here(RESULTS_FOLDER, "h_test_toddlers_shifts_al_num_shifts"))
```
...
rm(bm_test_toddlers_shifts_al_num_shifts_null)
gc()
```
...
bm_test_adults_shifts_al <- brm(
  ...
  data = filter(summarize_participant_test_both_trials_lc, age_cohort == "adults") %>%
    select(prop_exit, condition_c, num_shifts, participant_lab_id, lab_id),
  save_pars = save_pars(all = TRUE),
  warmup = 1000,
  iter = 20000,
  chains = 4,
...
bm_test_adults_shifts_al <- readRDS(here(model_fits_dir, "bm_test_adults_shifts_al.rds"))
```
...
```{r}
bm_test_adults_shifts_al_tidy_summary <- as_tibble(summary(bm_test_adults_shifts_al)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_adults_shifts_al_tidy_summary,
  file = here(RESULTS_FOLDER, "bm_test_adults_shifts_al_model_summary.rds")
)
```
...
bm_test_adults_shifts_al_condition_num_shifts_interaction <- bm_test_adults_shifts_al %>%
  spread_draws(`b_condition_c:num_shifts`, sigma) %>%
  mean_hdi(.width = 0.95)

# cache
saveRDS(bm_test_adults_shifts_al_condition_num_shifts_interaction,
        file = here(RESULTS_FOLDER, "bm_test_adults_shifts_al_condition_num_shifts_interaction.rds"))
```
...
```{r}
h_test_adults_shifts_al_interaction <- hypothesis(bm_test_adults_shifts_al, "condition_c:num_shifts = 0", class = "b")
h_test_adults_shifts_al_interaction
#plot(h_test_adults_shifts_al_num_shifts)
# evidence in favor of condition being different from zero
1 / h_test_adults_shifts_al_interaction$hypothesis$Evid.Ratio
saveRDS(h_test_adults_shifts_al_interaction, 
        file = here(RESULTS_FOLDER, "h_test_adults_shifts_al_interaction.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_adults_shifts_al_null <- readRDS(here(model_fits_dir, "bm_test_adults_shifts_al_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_adults_shifts_al <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_adults_shifts_al.rds"))
```

##### Number of Shifts

Alternative bayes factor computation for number of shifts (due to challenges with bridge sampling)

```{r}
h_test_adults_shifts_al_num_shifts <- hypothesis(bm_test_adults_shifts_al, "num_shifts = 0", class = "b")
h_test_adults_shifts_al_num_shifts
plot(h_test_adults_shifts_al_num_shifts)
# evidence in favor of condition being different from zero
1 / h_test_adults_shifts_al_num_shifts$hypothesis$Evid.Ratio
saveRDS(h_test_adults_shifts_al_num_shifts, 
        file = here(RESULTS_FOLDER, "h_test_adults_shifts_al_num_shifts.rds"))
```

Bridge sampling Bayes factor approach: Number of shifts Effect

```{r, eval = FIT_BAYES_MODELS}
bm_test_adults_shifts_al_num_shifts_null <- update(bm_test_adults_shifts_al, 
                                          formula = ~ . - num_shifts,
                                          cores = n_cores)
summary(bm_test_adults_shifts_al_num_shifts_null)

saveRDS(bm_test_adults_shifts_al_num_shifts_null, 
        file = here(model_fits_dir, "bm_test_adults_shifts_al_num_shifts_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_adults_shifts_al_num_shifts_null <- readRDS(here(model_fits_dir, "bm_test_adults_shifts_al_num_shifts_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_adults_shifts_al_num_shifts <- brms::bayes_factor(bm_test_adults_shifts_al, 
                                                           bm_test_adults_shifts_al_num_shifts_null)
 
saveRDS(test_m_comparison_adults_shifts_al_num_shifts, 
        file = here(RESULTS_FOLDER, "test_m_comparison_adults_shifts_al_num_shifts.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_adults_shifts_al_num_shifts <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_adults_shifts_al_num_shifts.rds"))
```
...
rm(bm_test_adults_shifts_al_num_shifts_null)
gc()
```

#### Creating results tables for Bayesian mixed effects models of AL as a function of the number of gaze shifts during location change of the mouse

```{r}
table_names_al_shifts <- c("bm_test_toddlers_shifts_al_tidy_summary", "bm_test_adults_shifts_al_tidy_summary")

# Create a function to extract age cohort and test phase from the table names
extract_metadata_fam <- function(table_names_al_shifts) {
  parts <- strsplit(table_names_al_shifts, "_")[[1]]
  list(
    measure = parts[3], # e.g., "test"
    age_cohort = parts[4]  # e.g., "adults"
  )
...
# Combine all tables into one
combined_brm_results_al_shifts <- table_names_al_shifts %>%
  map_df(~ {
    metadata <- extract_metadata_fam(.x)
    get(.x) %>% 
      mutate(
  ...
  mutate(term = term %>%
    str_replace_all("condition_c", "Condition") %>%
    str_replace_all("num_shifts", "Number Shifts"))


# View the combined table
print(combined_brm_results_al_shifts)

saveRDS(combined_brm_results_al_shifts,
  file = here(RESULTS_FOLDER, "combined_brm_results_al_shifts.rds")
)
```
...
AL_diff_mouse_bear <- ggplot(summarize_participant_test_both_trials_lc,
  ...
  geom_hline(yintercept = 0.5, linetype = "dashed") +
  scale_color_brewer(palette = "Set1") +
  scale_fill_brewer(palette = "Set1") +  xlab("Difference in looking time to mouse and bear\nduring location change of mouse (in ms)") +
...
#### Toddlers
```{r, eval = FIT_BAYES_MODELS}
# convert diff_mouse_bear in s

summarize_participant_test_both_trials_lc <- summarize_participant_test_both_trials_lc %>%
  mutate(diff_mouse_bear_s = diff_mouse_bear / 1000)

bm_test_toddlers_diff_mouse_bear_al <- brm_default(
  prop_exit ~ 1 + condition_c + diff_mouse_bear_s + condition_c : diff_mouse_bear_s + 
    (1 + condition_c + diff_mouse_bear_s + condition_c : diff_mouse_bear_s | lab_id) +
    (1 + condition_c | participant_lab_id),
  family = gaussian,
  prior = priors_default,
  data = filter(summarize_participant_test_both_trials_lc, age_cohort == "toddlers")
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_toddlers_diff_mouse_bear_al <- readRDS(here(model_fits_dir, "bm_test_toddlers_diff_mouse_bear_al.rds"))
```

Organize table in a tidy format

```{r}
bm_test_toddlers_diff_mouse_bear_al_tidy_summary <- as_tibble(summary(bm_test_toddlers_diff_mouse_bear_al)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_toddlers_diff_mouse_bear_al_tidy_summary,
  file = here(RESULTS_FOLDER, "bm_test_toddlers_diff_mouse_bear_al_model_summary.rds")
)
```
...
bm_test_toddlers_diff_mouse_bear_al_diff_effect <- bm_test_toddlers_diff_mouse_bear_al %>%
  spread_draws(b_diff_mouse_bear_s, sigma) %>%
  mean_hdi(.width = 0.95)
...
bm_test_toddlers_diff_mouse_bear_al_condition_diff_interaction <- bm_test_toddlers_diff_mouse_bear_al %>%
  spread_draws(`b_condition_c:diff_mouse_bear_s`, sigma) %>%
  mean_hdi(.width = 0.95)
...
Bridge sampling Bayes factor approach: Condition diff_mouse_bear Interaction effect

```{r, eval = FIT_BAYES_MODELS}
bm_test_toddlers_diff_mouse_bear_al_null <- update(bm_test_toddlers_diff_mouse_bear_al, 
                                                   formula = ~ . - condition_c:diff_mouse_bear_s, 
                                                   cores = n_cores)
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_toddlers_diff_mouse_bear_al_null <- readRDS(here(model_fits_dir, "bm_test_toddlers_diff_mouse_bear_al_null.rds"))
```
...
# Warnung: logml could not be estimated within maxiter, rerunning with adjusted starting value. 
# Estimate might be more variable than usual.
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_toddlers_diff_mouse_bear_al <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_toddlers_diff_mouse_bear_al.rds"))
```

Bridge sampling: diff_mouse_bear effect

```{r, eval = FIT_BAYES_MODELS}
bm_test_toddlers_diff_mouse_bear_al_diff_mouse_bear_null <- update(bm_test_toddlers_diff_mouse_bear_al, 
                                                 formula = ~ . - diff_mouse_bear_s,
                                                 cores = n_cores)
summary(bm_test_toddlers_diff_mouse_bear_al_diff_mouse_bear_null)

saveRDS(bm_test_toddlers_diff_mouse_bear_al_diff_mouse_bear_null,
        file = here(model_fits_dir, "bm_test_toddlers_diff_mouse_bear_al_diff_mouse_bear_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_toddlers_diff_mouse_bear_al_diff_mouse_bear_null <- readRDS(here(model_fits_dir, "bm_test_toddlers_diff_mouse_bear_al_diff_mouse_bear_null.rds"))
```
...
test_m_comparison_toddlers_diff_mouse_bear_al_diff_mouse_bear_effect <- brms::bayes_factor(bm_test_toddlers_diff_mouse_bear_al, bm_test_toddlers_diff_mouse_bear_al_diff_mouse_bear_null)

saveRDS(test_m_comparison_toddlers_diff_mouse_bear_al_diff_mouse_bear_effect,
        file = here(RESULTS_FOLDER, "test_m_comparison_toddlers_diff_mouse_bear_al_diff_mouse_bear_effect.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_toddlers_diff_mouse_bear_al_diff_mouse_bear_effect <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_toddlers_diff_mouse_bear_al_diff_mouse_bear_effect.rds"))
```

Alternative bayes factor computation for interaction effect (due to challenges with bridge sampling)

```{r}
h_test_toddlers_diff_mouse_bear_al <- hypothesis(bm_test_toddlers_diff_mouse_bear_al, "condition_c:diff_mouse_bear_s = 0", class = "b")
h_test_toddlers_diff_mouse_bear_al
# evidence in favor of interaction being different from zero
1 / h_test_toddlers_diff_mouse_bear_al$hypothesis$Evid.Ratio
saveRDS(h_test_toddlers_diff_mouse_bear_al, 
        file = here(RESULTS_FOLDER, "h_test_toddlers_diff_mouse_bear_al.rds"))
```

Alternative bayes factor computation for diff_mouse_bear effect (due to challenges with bridge sampling)

```{r}
h_test_toddlers_diff_mouse_bear_al_diff_mouse_bear <- hypothesis(bm_test_toddlers_diff_mouse_bear_al, "diff_mouse_bear_s = 0", class = "b")
h_test_toddlers_diff_mouse_bear_al_diff_mouse_bear
# evidence in favor of interaction being different from zero
1 / h_test_toddlers_diff_mouse_bear_al_diff_mouse_bear$hypothesis$Evid.Ratio
saveRDS(h_test_toddlers_diff_mouse_bear_al_diff_mouse_bear, 
        file = here(RESULTS_FOLDER, "h_test_toddlers_diff_mouse_bear_al_diff_mouse_bear.rds"))
```
...
rm(bm_test_toddlers_diff_mouse_bear_al_diff_mouse_bear_null)
gc()
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_adults_diff_mouse_bear_al <- readRDS(here(model_fits_dir, "bm_test_adults_diff_mouse_bear_al.rds"))
```

Organize table in a tidy format

```{r}
bm_test_adults_diff_mouse_bear_al_tidy_summary <- as_tibble(summary(bm_test_adults_diff_mouse_bear_al)$fixed,rownames=NA) %>% 
  rownames_to_column("term") %>% 
  rename(
    estimate = Estimate,
    est_error = Est.Error,
...
saveRDS(bm_test_adults_diff_mouse_bear_al_tidy_summary,
  file = here(RESULTS_FOLDER, "bm_test_adults_diff_mouse_bear_al_model_summary.rds")
)
```
...
Bridge sampling: interaction effect
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_adults_diff_mouse_bear_al_null <- readRDS(here(model_fits_dir, "bm_test_adults_diff_mouse_bear_al_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_adults_diff_mouse_bear_al <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_adults_diff_mouse_bear_al.rds"))
```

Alternative bayes factor computation for interaction effect (due to challenges with bridge sampling)

```{r}
h_test_adults_diff_mouse_bear_al <- hypothesis(bm_test_adults_diff_mouse_bear_al, "condition_c:diff_mouse_bear_s = 0", class = "b")
h_test_adults_diff_mouse_bear_al
# evidence in favor of interaction being different from zero
1 / h_test_adults_diff_mouse_bear_al$hypothesis$Evid.Ratio
saveRDS(h_test_adults_diff_mouse_bear_al, 
        file = here(RESULTS_FOLDER, "h_test_adults_diff_mouse_bear_al.rds"))
```

Bridge sampling: diff_mouse_bear effect

```{r, eval = FIT_BAYES_MODELS}
bm_test_adults_diff_mouse_bear_al_diff_mouse_bear_null <- update(bm_test_adults_diff_mouse_bear_al, 
                                                 formula = ~ . - diff_mouse_bear_s,
                                                 cores = n_cores)
summary(bm_test_adults_diff_mouse_bear_al_diff_mouse_bear_null)

saveRDS(bm_test_adults_diff_mouse_bear_al_diff_mouse_bear_null,
        file = here(model_fits_dir, "bm_test_adults_diff_mouse_bear_al_diff_mouse_bear_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_adults_diff_mouse_bear_al_diff_mouse_bear_null <- readRDS(here(model_fits_dir, "bm_test_adults_diff_mouse_bear_al_diff_mouse_bear_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
# Warning: logml could not be estimated within maxiter, rerunning with adjusted starting value. Estimate might be more variable than usual.Iteration: 1
test_m_comparison_adults_diff_mouse_bear_al_diff_mouse_bear_effect <- brms::bayes_factor(bm_test_adults_diff_mouse_bear_al, bm_test_adults_diff_mouse_bear_al_diff_mouse_bear_null)

saveRDS(test_m_comparison_adults_diff_mouse_bear_al_diff_mouse_bear_effect,
        file = here(RESULTS_FOLDER, "test_m_comparison_adults_diff_mouse_bear_al_diff_mouse_bear_effect.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_adults_diff_mouse_bear_al_diff_mouse_bear_effect <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_adults_diff_mouse_bear_al_diff_mouse_bear_effect.rds"))
```

Alternative bayes factor computation for diff_mouse_bear effect (due to challenges with bridge sampling)

```{r}
h_test_adults_diff_mouse_bear_al_diff_mouse_bear <- hypothesis(bm_test_adults_diff_mouse_bear_al, "diff_mouse_bear_s = 0", class = "b")
h_test_adults_diff_mouse_bear_al_diff_mouse_bear
# evidence in favor of interaction being different from zero
1 / h_test_adults_diff_mouse_bear_al_diff_mouse_bear$hypothesis$Evid.Ratio
saveRDS(h_test_adults_diff_mouse_bear_al_diff_mouse_bear, 
        file = here(RESULTS_FOLDER, "h_test_adults_diff_mouse_bear_al_diff_mouse_bear.rds"))
```
...
rm(bm_test_adults_diff_mouse_bear_al_diff_mouse_bear_null)
gc()
```
...
```{r}
table_names_al_diff_mouse_bear <- c("bm_test_toddlers_diff_mouse_bear_al_tidy_summary", "bm_test_adults_diff_mouse_bear_al_tidy_summary")

# Create a function to extract age cohort and test phase from the table names
extract_metadata_fam <- function(table_names_al_diff_mouse_bear) {
  parts <- strsplit(table_names_al_diff_mouse_bear, "_")[[1]]
  list(
    measure = parts[3], # e.g., "test"
    age_cohort = parts[4]  # e.g., "adults"
  )
...
# Combine all tables into one
combined_brm_results_al_diff_mouse_bear <- table_names_al_diff_mouse_bear %>%
  map_df(~ {
    metadata <- extract_metadata_fam(.x)
    get(.x) %>% 
      mutate(
  ...
  mutate(term = term %>%
    str_replace_all("condition_c", "Condition") %>%
    str_replace_all("diff_mouse_bear_s", "Difference Mouse-Bear"))

# View the combined table
print(combined_brm_results_al_diff_mouse_bear)

saveRDS(combined_brm_results_al_diff_mouse_bear,
  file = here(RESULTS_FOLDER, "combined_brm_results_al_diff_mouse_bear.rds")
)
```
...
saveRDS(bm_distraction_toddlers,
  file = here(model_fits_dir, "bm_distraction_toddlers.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_distraction_toddlers <- readRDS(here(model_fits_dir, "bm_distraction_toddlers.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_distraction_toddlers_null <- readRDS(here(model_fits_dir, "bm_distraction_toddlers_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_toddlers_distraction <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_toddlers_distraction.rds"))
```
...
saveRDS(bm_distraction_adults,
  file = here(model_fits_dir, "bm_distraction_adults.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_distraction_adults <- readRDS(here(model_fits_dir, "bm_distraction_adults.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_distraction_adults_null <- readRDS(here(model_fits_dir, "bm_distraction_adults_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_adults_distraction <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_adults_distraction.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_al_cohort <- readRDS(here(model_fits_dir, "bm_al_cohort.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_al_cohort_null <- readRDS(here(model_fits_dir, "bm_al_cohort_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_al_cohort <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_al_cohort.rds"))
```
...
summarize_participant_test_first_trial_all_fam_trials <- summarize_participant_test_first_trial_all_fam_trials %>% 
  left_join(location_bear_mouse_fam_test)

summarize_participant_test_first_trial_wo_NA_location <- summarize_participant_test_first_trial_all_fam_trials %>% 
...
ggsave(here(paper_path, "FigureS6.png"),
  bg = "white", width = 9, height = 6
)
```
...
bm_test_toddlers_location_al <- brm_default(
  prop_exit ~ 1 + condition_c + location_c + condition_c : location_c +
    (1 + condition_c + location_c + condition_c : location_c | lab_id),
  family = gaussian,
  prior = priors_default,
...
saveRDS(bm_test_toddlers_location_al, 
        file = here(model_fits_dir, "bm_test_toddlers_location_al.rds"))
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_toddlers_location_al <- readRDS(here(model_fits_dir, "bm_test_toddlers_location_al.rds"))
```
...
Bridge sampling Bayes factor approach: interaction effect
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_toddlers_location_al_null <- readRDS(here(model_fits_dir, "bm_test_toddlers_location_al_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_toddlers_location_al <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_toddlers_location_al.rds"))
```

Bridge sampling Bayes factor approach: location effect

```{r, eval = FIT_BAYES_MODELS}
bm_test_toddlers_location_al_location_c_null <- update(bm_test_toddlers_location_al, 
                                          formula = ~ . - location_c,
                                          cores = n_cores)
summary(bm_test_toddlers_location_al_location_c_null)

saveRDS(bm_test_toddlers_location_al_location_c_null, 
        file = here(model_fits_dir, "bm_test_toddlers_location_al_location_c_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_toddlers_location_al_location_c_null <- readRDS(here(model_fits_dir, "bm_test_toddlers_location_al_location_c_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_toddlers_location_al_location_c <- brms::bayes_factor(bm_test_toddlers_location_al, 
                                                           bm_test_toddlers_location_al_location_c_null)

saveRDS(test_m_comparison_toddlers_location_al_location_c, 
        file = here(RESULTS_FOLDER, "test_m_comparison_toddlers_location_al_location_c.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_toddlers_location_al_location_c <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_toddlers_location_al_location_c.rds"))
```
...
rm(bm_test_toddlers_location_al_location_c_null)
gc()
```
...
saveRDS(bm_test_adults_location_al, 
        file = here(model_fits_dir, "bm_test_adults_location_al.rds"))
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_adults_location_al <- readRDS(here(model_fits_dir, "bm_test_adults_location_al.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_adults_location_al_null <- readRDS(here(model_fits_dir, "bm_test_adults_location_al_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_adults_location_al <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_adults_location_al.rds"))
```

Bridge sampling Bayes factor approach: location effect

```{r, eval = FIT_BAYES_MODELS}
bm_test_adults_location_al_location_c_null <- update(bm_test_adults_location_al, 
                                          formula = ~ . - location_c,
                                          cores = n_cores)
summary(bm_test_adults_location_al_location_c_null)

saveRDS(bm_test_adults_location_al_location_c_null, 
        file = here(model_fits_dir, "bm_test_adults_location_al_location_c_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_adults_location_al_location_c_null <- readRDS(here(model_fits_dir, "bm_test_adults_location_al_location_c_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
test_m_comparison_adults_location_al_location_c <- brms::bayes_factor(bm_test_adults_location_al, 
                                                           bm_test_adults_location_al_location_c_null)

saveRDS(test_m_comparison_adults_location_al_location_c, 
        file = here(RESULTS_FOLDER, "test_m_comparison_adults_location_al_location_c.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_adults_location_al_location_c <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_adults_location_al_location_c.rds"))
```
...
rm(bm_test_adults_location_al_location_c_null)
gc()
```
...
bm_fam_toddlers_location_al <- brm(
  prop_exit_adj ~ 1 + prev_location_c + familiarization_trial_num_c + prev_location_c : familiarization_trial_num_c +
    (1 | participant_lab_id) + 
    (1 + prev_location_c + familiarization_trial_num_c + prev_location_c : familiarization_trial_num_c | lab_id),
  family = gaussian,
  prior = priors,
  save_pars = save_pars(all = TRUE),
...
saveRDS(bm_fam_toddlers_location_al, 
        file = here(model_fits_dir, "bm_fam_toddlers_location_al.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_fam_toddlers_location_al <- readRDS(here(model_fits_dir, "bm_fam_toddlers_location_al.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_fam_toddlers_location_al_null <- readRDS(here(model_fits_dir, "bm_fam_toddlers_location_al_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
fam_m_comparison_toddlers_location_al <- readRDS(here(RESULTS_FOLDER, "fam_m_comparison_toddlers_location_al.rds"))
```

Bridge sampling Bayes factor approach: prev_location effect

```{r, eval = FIT_BAYES_MODELS}
bm_fam_toddlers_location_al_prev_location_c_null <- update(bm_fam_toddlers_location_al, 
                                          formula = ~ . - prev_location_c,
                                          cores = n_cores)
summary(bm_fam_toddlers_location_al_prev_location_c_null)

saveRDS(bm_fam_toddlers_location_al_prev_location_c_null, 
        file = here(model_fits_dir, "bm_fam_toddlers_location_al_prev_location_c_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_fam_toddlers_location_al_prev_location_c_null <- readRDS(here(model_fits_dir, "bm_fam_toddlers_location_al_prev_location_c_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
fam_m_comparison_toddlers_location_al_prev_location_c <- brms::bayes_factor(bm_fam_toddlers_location_al, 
                                                           bm_fam_toddlers_location_al_prev_location_c_null)

saveRDS(fam_m_comparison_toddlers_location_al_prev_location_c, 
        file = here(RESULTS_FOLDER, "fam_m_comparison_toddlers_location_al_prev_location_c.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
fam_m_comparison_toddlers_location_al_prev_location_c <- readRDS(here(RESULTS_FOLDER, "fam_m_comparison_toddlers_location_al_prev_location_c.rds"))
```

Alternative bayes factor computation for interaction effect (due to challenges with bridge sampling)

```{r}
h_fam_toddlers_location_al_interaction <- hypothesis(bm_fam_toddlers_location_al, "prev_location_c:familiarization_trial_num_c = 0", class = "b")
h_fam_toddlers_location_al_interaction
# evidence in favor of interaction being different from zero
1 / h_fam_toddlers_location_al_interaction$hypothesis$Evid.Ratio
saveRDS(h_fam_toddlers_location_al_interaction, 
        file = here(RESULTS_FOLDER, "h_fam_toddlers_location_al_interaction.rds"))
```

Alternative bayes factor computation for prev location effect (due to challenges with bridge sampling)

```{r}
h_fam_toddlers_location_al_prev_location <- hypothesis(bm_fam_toddlers_location_al, "prev_location_c = 0", class = "b")
h_fam_toddlers_location_al_prev_location
# evidence in favor of interaction being different from zero
1 / h_fam_toddlers_location_al_prev_location$hypothesis$Evid.Ratio
saveRDS(h_fam_toddlers_location_al_prev_location, 
        file = here(RESULTS_FOLDER, "h_fam_toddlers_location_al_prev_location.rds"))
```
...
rm(bm_fam_toddlers_location_al_prev_location_c_null)
gc()
```
...
saveRDS(bm_fam_adults_location_al, 
        file = here(model_fits_dir, "bm_fam_adults_location_al.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_fam_adults_location_al <- readRDS(here(model_fits_dir, "bm_fam_adults_location_al.rds"))
```
...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_fam_adults_location_al_null <- readRDS(here(model_fits_dir, "bm_fam_adults_location_al_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
fam_m_comparison_adults_location_al <- readRDS(here(RESULTS_FOLDER, "fam_m_comparison_adults_location_al.rds"))
```

Bridge sampling Bayes factor approach: prev_location effect

```{r, eval = FIT_BAYES_MODELS}
bm_fam_adults_location_al_prev_location_c_null <- update(bm_fam_adults_location_al, 
                                          formula = ~ . - prev_location_c,
                                          cores = n_cores)
summary(bm_fam_adults_location_al_prev_location_c_null)

saveRDS(bm_fam_adults_location_al_prev_location_c_null, 
        file = here(model_fits_dir, "bm_fam_adults_location_al_prev_location_c_null.rds"))
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_fam_adults_location_al_prev_location_c_null <- readRDS(here(model_fits_dir, "bm_fam_adults_location_al_prev_location_c_null.rds"))
```

```{r, eval = COMPUTE_BAYES_FACTORS}
fam_m_comparison_adults_location_al_prev_location_c <- brms::bayes_factor(bm_fam_adults_location_al, 
                                                           bm_fam_adults_location_al_prev_location_c_null)

saveRDS(fam_m_comparison_adults_location_al_prev_location_c, 
        file = here(RESULTS_FOLDER, "fam_m_comparison_adults_location_al_prev_location_c.rds"))
```

```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
fam_m_comparison_adults_location_al_prev_location_c <- readRDS(here(RESULTS_FOLDER, "fam_m_comparison_adults_location_al_prev_location_c.rds"))
```

Alternative bayes factor computation for interaction effect (due to challenges with bridge sampling)

```{r}
h_fam_adults_location_al_interaction <- hypothesis(bm_fam_adults_location_al, "prev_location_c:familiarization_trial_num_c = 0", class = "b")
h_fam_adults_location_al_interaction
# evidence in favor of interaction being different from zero
1 / h_fam_adults_location_al_interaction$hypothesis$Evid.Ratio
saveRDS(h_fam_adults_location_al_interaction, 
        file = here(RESULTS_FOLDER, "h_fam_adults_location_al_interaction.rds"))
```

Alternative bayes factor computation for prev location effect (due to challenges with bridge sampling)

```{r}
h_fam_adults_location_al_prev_location <- hypothesis(bm_fam_adults_location_al, "prev_location_c = 0", class = "b")
h_fam_adults_location_al_prev_location
# evidence in favor of interaction being different from zero
1 / h_fam_adults_location_al_prev_location$hypothesis$Evid.Ratio
saveRDS(h_fam_adults_location_al_prev_location, 
        file = here(RESULTS_FOLDER, "h_fam_adults_location_al_prev_location.rds"))
```
...
rm(bm_fam_adults_location_al_prev_location_c_null)
