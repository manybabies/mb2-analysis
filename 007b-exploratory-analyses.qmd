...
```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_test_shifts_age_cohort_condition_null <- readRDS(here(model_fits_dir, "bm_test_shifts_age_cohort_condition_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_m_comparison_shifts_age_cohort_condition <- readRDS(here(RESULTS_FOLDER, "test_m_comparison_shifts_age_cohort_condition.rds"))
```
