...
bm_aoi_toddlers_both_order <- brm_default(
  prop_exit ~ 1 + condition_c + test_trial_num + condition_c : test_trial_num + 
    ...
    (1 | participant_lab_id), # we decided not to include random slopes because we have only 2 observations per participant (to avoid over-parameterization)
  family = gaussian,
  prior = priors_default,
  data = filter(summarize_participant_test_both_trials, age_cohort == "toddlers")
)
...
saveRDS(bm_box_adults_interaction_null,
  file = here(model_fits_dir, "bm_box_adults_interaction_null.rds")
)
```

```{r, eval = !FIT_BAYES_MODELS}
# if model is not fit, load from disk
bm_box_adults_interaction_null <- readRDS(here(model_fits_dir, "bm_box_adults_interaction_null.rds"))
```
...
```{r, eval = !COMPUTE_BAYES_FACTORS}
# if BF is not computed, load from disk
test_box_m_comparison_PTL_adults_interaction <- readRDS(here(RESULTS_FOLDER, "test_box_m_comparison_PTL_adults_interaction.rds"))
```
...
bm_test_toddlers_diff_mouse_bear_al <- brm_default(
  prop_exit ~ 1 + condition_c + diff_mouse_bear_s + condition_c * diff_mouse_bear_s + 
    (1 + condition_c * diff_mouse_bear_s | lab_id) +
    (1 + condition_c + diff_mouse_bear_s + condition_c * diff_mouse_bear_s | participant_lab_id),
  family = gaussian,
  prior = priors_default,
  data = filter(summarize_participant_test_both_trials_lc, age_cohort == "toddlers")
)
