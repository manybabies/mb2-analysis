...
bm_aoi_toddlers_second_trial <- brm_default(
  second_trial_prop_exit ~ 1 + condition_c + first_trial_prop_exit + condition_c * first_trial_prop_exit +
...
bm_aoi_toddlers_second_trial_null <- update(bm_aoi_toddlers_second_trial, 
                                            formula = ~ . - condition_c * first_trial_prop_exit,
                                            cores = n_cores)
...
```
### 2.d. Location of bear during last familiarization trial and final location of mouse in first test trial
...
# Renee's comment on the yo-yo effect: "
...
model_location <- lmer(
  prop_exit ~ location * age_cohort * condition + 
    (1 | participant_id),
  data = summarize_participant_test_first_trial_wo_NA_location
)
...
library(emmeans)
emm <- emmeans(model_location, ~ location * age_cohort * condition)
plot(emm)
```
