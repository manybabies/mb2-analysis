...
compute_shifts_and_fixation_times <- function(data) {
  ...
  for (condition in names(location_changes)) {
  ...
  filtered_data <- data %>%
    ...
    arrange(participant_lab_id, trial_num, t_zeroed)

    # Calculate number of shifts and fixation times on mouse and bear during location change of mouse
  condition_results <- filtered_data %>% #filter(participant_lab_id=="CogConcordia_Adult10-adults"& trial_num==5) %>%      #select(participant_lab_id, trial_num, condition, lab_id, age_cohort, t_zeroed, aoi_bearmouse_special) %>% 
    group_by(participant_lab_id, condition, trial_num, lab_id, age_cohort) %>%
    mutate(
      shift_test = (lag(aoi_bearmouse_special)),
      shift = (aoi_bearmouse_special != lag(aoi_bearmouse_special)) & !is.na(lag(aoi_bearmouse_special))
      #fixation_time = lead(t_zeroed, default = last(t_zeroed)) - t_zeroed
    ) %>%
    summarise(
...
summarize_overall_test_shifts <- d_location_change %>%
  group_by(age_cohort, condition) %>%
  summarise(N = sum(!is.na(num_shifts)),
...
ggplot(d_location_change,aes(condition,num_shifts,color=condition))+
...
brms::bayes_factor(bm_test_shifts_age_cohort, null_test_shifts_age_cohort)
```

### 5.2. Infants gaze shifts during location change as a function of AL
...
ggplot(d_location_change,aes(condition,diff_mouse_bear,color=condition))+
  #geom_errorbar(aes(ymin=lower_ci,ymax=upper_ci),width=0
  geom_point(position = position_jitter(seed = 2, width = 0.4)) +
  geom_violin(alpha = 0.5) +
...
ggplot(d_location_change_long,aes(target,looking_time,color=target))+
  #geom_errorbar(aes(ymin=lower_ci,ymax=upper_ci),width=0
  geom_point(position = position_jitter(seed = 2, width = 0.4)) +
  geom_violin(alpha = 0.5) +
  #geom_jitter(width=0.5)+
