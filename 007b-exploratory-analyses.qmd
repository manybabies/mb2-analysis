...
ggplot(d_location_change,aes(age_cohort,num_shifts, color=age_cohort))+
...
### 5.2. Fixation times of bear and mouse during location change of mouse as a function of AL
...
# plotting AL and difference in looking times to mouse and bear

ggplot(summarize_participant_test_both_trials_lc,aes(prop_exit,diff_mouse_bear,color=condition))+
  geom_point() +
  geom_hline(yintercept=0,linetype="dashed")+
  geom_smooth(method='lm')+
  facet_wrap(condition~age_cohort)+
  ...
  scale_fill_brewer(palette="Set1")+
  theme(legend.position="none")+
  xlab("AL in first test trial")+
  ylab("Difference in looking time between mouse and bear")

```


### 5.3. NUmber of shifts during location change of mouse as a function of AL
...
ggplot(summarize_participant_test_both_trials_lc,aes(prop_exit,num_shifts,color=condition))+
  geom_point() +
  geom_smooth(method='lm')+
  facet_wrap(condition~age_cohort)+
  theme_cowplot()+
