...
bm_test_shifts_age_cohort <- brm(num_shifts ~ 1+condition_c*age_cohort_c+(1+condition_c*age_cohort_c|lab_id),
         family=gaussian,
         prior = priors,
         save_pars = save_pars(all = TRUE),
...
```{r}
h_test_shifts_age_cohort <- hypothesis(bm_test_shifts_age_cohort, "condition_c = 0")
...
```{r}
null_test_shifts_age_cohort <-  update(bm_test_shifts_age_cohort, formula = ~ .-condition_c)
...
looking_time_location_change_bear_mouse <-  ggplot(d_location_change_long,aes(target,looking_time,color=condition))+
...
bm_test_diff_mouse_bear_al <- brm(prop_exit ~ 1+condition_c*diff_mouse_bear+(1+condition_c*diff_mouse_bear|lab_id)+(1+condition_c*diff_mouse_bear|participant_lab_id), 
         family=gaussian,
         prior = priors,
         save_pars = save_pars(all = TRUE),
        filter(summarize_participant_test_both_trials_lc),
