...
# data_preprocessed <- arrow::open_dataset(here(INTERMEDIATE_FOLDER, INTERMEDIATE_006_par),  format = "parquet") %>% collect() # if we want data post-exclusion
data_preprocessed <- arrow::open_dataset(here(INTERMEDIATE_FOLDER, INTERMEDIATE_006b_par), format = "parquet") %>% collect() # if we want data pre-exclusion
```
