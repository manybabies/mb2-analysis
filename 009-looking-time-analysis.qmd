...
# averaging just the congruent vs. incongruent conditions
avg_lt_data_outcome_voe_criterion <- total_participant_lt_data_voe_criterion %>%
  ungroup() %>%
  group_by(age_cohort,outcome) %>%
  summarize(
    mean_looking_time = mean(total_cumulative_looking_time),
    sd_looking_time = sd(total_cumulative_looking_time),
    n = n(),
...
### For ICIS 2026 Abstract: Simple inferential stats

```{r}
# t-test for congruence condition
t.test(
  total_participant_lt_data_voe_criterion %>% ungroup() %>% filter(age_cohort=="toddlers" & outcome=="congruent") %>% select(total_cumulative_looking_time),
  total_participant_lt_data_voe_criterion %>% ungroup() %>% filter(age_cohort=="toddlers" & outcome=="incongruent") %>% select(total_cumulative_looking_time),
  var.equal = TRUE)

#interaction
total_participant_lt_data_voe_criterion <- total_participant_lt_data_voe_criterion %>%
  mutate(
    outcome_c = ifelse(outcome=="congruent",0.5, -0.5),
    condition_c = ifelse(condition=="knowledge",0.5, -0.5)
  )
m <- lm(total_cumulative_looking_time ~ outcome_c * condition_c, data=filter(total_participant_lt_data_voe_criterion,age_cohort=="toddlers"))
summary(m)
```
...
ggplot(total_participant_lt_data_voe_no_crit,aes(log_looking_time, color=outcome))+
  geom_density()+
  facet_wrap(~condition+age_cohort)
ggplot(filter(total_participant_lt_data_voe_no_crit,age_cohort=="toddlers"),aes(total_cumulative_looking_time, fill=outcome))+
  geom_histogram()+
  facet_wrap(outcome~condition)
