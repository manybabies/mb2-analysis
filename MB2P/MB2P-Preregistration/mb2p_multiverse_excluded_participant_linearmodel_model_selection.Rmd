...
```{r Ploting all 72 datasets and grand average, echo=TRUE, include=TRUE, eval=CREATE_MULTIVERSE_PLOTS,out.width='90%'}
...
summarize_data_within_lab_by_multiverse_label <- summarized_data_by_lab_and_dataset_id |> 
    group_by(t_norm_downsampled, multiverse_label, condition, outcome, age_cohort) |>  
  summarize(
    mean_pupil_z = mean(grand_averageZ, na.rm = TRUE), .groups = "drop"
  ) |> 
  ungroup() |> 
  unite("multiverse_id_wcond", multiverse_label,condition, outcome, age_cohort, sep = "_", remove = FALSE)

summarized_data_by_lab_and_dataset_id_mins <- summarize_data_within_lab_by_multiverse_label |> 
  group_by(t_norm_downsampled, condition, outcome, age_cohort) |>  
  summarize(
    N=n(),
...
#### Spaghetti plot to show effect for each multiverse dataset

```{r}

#spaghetti plot
ggplot(summarize_data_within_lab_by_multiverse_label,aes(t_norm_downsampled,y=mean_pupil_z))+
