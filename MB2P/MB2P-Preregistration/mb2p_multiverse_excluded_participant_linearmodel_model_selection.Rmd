...
ggplot(lmer_multiverse_combos_obs,aes(x=df1_extreme_values,y=n_obs))+
  geom_point()+
  geom_line(aes(group=1))+
  facet_grid(df4_baseline_correction+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
  ylab("Number of observations in LMER model")

ggplot(lmer_multiverse_combos_obs,aes(x=df1_extreme_values,y=n_labs))+
  geom_point()+
  geom_line(aes(group=1))+
  facet_grid(df4_baseline_correction+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
...
ggplot(lm_threewayinteraction_multiverse_results,aes(x=df4_baseline_correction,y=estimate))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
