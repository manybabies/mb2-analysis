...
#! Double-check the choice in time-window. Do some models require time for the entirety of the second test trial?
...
#! This would be a good place to do the z-transformation.
...
rm("data_plausible.SMI.Tobii", "data_plausible.Eyelink")
...
) |> nest(data = -df1_extreme_values) #! What does this do?

# Prelim check:
data_pupillometry |> 
  filter(eyetracker_type != "EyeLink") |>
  summarize(range_min = min(pupil_left, na.rm=T),
            range_max = max(pupil_left, na.rm=T))

#! Why are there still such large values even with Eyelink-data removed? Likely Nimegen issue -> solved.

# scatterplot #! This needs to be fixed once the z-transformation is done.
data_pupillometry |> 
  #filter(eyetracker_type != "EyeLink") |>
...
data_avg <- data_df2 |> 
  mutate(data = map(data, \(d) { #! What does \(d) do?
    d |> 
...
# plotting #! Will need to create these plots for each of the final (72?) datasets.
data_df4 |> 
  pull(data) |> 
...
```{r Ploting all 72 datasets, echo=TRUE, include=TRUE, out.width='90%'}
...
```{r modelling, echo=FALSE, include=TRUE, out.width='90%'}
...
run_model_lmer <- function(df) {
  if (is.data.frame(df) && n_distinct(df$condition_c) > 1 & 
    ...
    lmer(Average ~ condition_c * outcome_c * age_cohort_c + (1|lab_id), data = df.short) #! Pending vote; include lab-id here. too! This is where we can include AL as fixed effect.
  } else {
    # Return NULL if conditions aren't met
    NULL
  }
...
extract_model_metrics <- function(model) {
  if (!is.null(model)) {
    ...
    coef_df <- coef_df %>%
      filter(term %in% c("condition_c", "outcome_c", "age_cohort_c")) %>% #! Here we need only one coefficient, i.e., the one for the 3-way interaction?
...
# It looks like all the model values are identical with filtered datasets (here no effect of df1 and df2?)?
# Likewise, within unfiltered, the two plausible are identical and the two implausible are identical.
