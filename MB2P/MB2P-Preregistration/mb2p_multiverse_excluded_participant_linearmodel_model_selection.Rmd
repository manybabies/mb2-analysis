...
```{r}
#load in helper file with ages
#this helped file was created by pulling key demographic columns from INTERMEDIATE_006
ages <- read.csv(here("MB2P","MB2P-Preregistration","helper_data","mb2_age_demographics_helper_006.csv"))

descriptives <- data_pupillometry |>
  # apply core filtering steps
  filter(pilot == "no",
      session_error == "noerror",
      age_exclusion == "no",
      sufficient_fam_trials == 1) |> 
  distinct(lab_id,participant_lab_id,participant_gender,age_cohort,condition,outcome) |> 
  left_join(ages) |>
  group_by(age_cohort) |> 
  summarize(
    N=n(),
    age_years_m = mean(age_years_n,na.rm=TRUE),
    age_years_sd = sd(age_years_n,na.rm=TRUE),
    age_years_min = min(age_years_n,na.rm=TRUE),
    age_years_max = max(age_years_n,na.rm=TRUE),
    age_months_m = mean(age_mo,na.rm=TRUE),
    age_months_sd = sd(age_mo,na.rm=TRUE),
    age_months_min = min(age_mo,na.rm=TRUE),
    age_months_max = max(age_mo,na.rm=TRUE),
    percent_female = sum(
      participant_gender == "woman" | participant_gender == "girl"
    ) / sum(!is.na(participant_gender))
  )
...
data_pupillometry |>
  filter(average > 1.8) |>
  dplyr::select(lab_id, average, eyetracker_type)

# We may need to revisit the 2mm criterion given that some data are within 1.8 and 2. However, this really only affects 200ish data points of +245k...
```
...
walk2(plots, labels, ~ ggsave(
  filename = file.path(plot_output_path, paste0(.y, ".png")),
  plot = .x,
...
#### New Min-Max plot after first averaging within lab
...
```{r}
#"best" variant, This plot is used in the abstract.
...
#first, select the best multiverse variant data
best_multiverse_data <- data_for_plotting_df5 |> 
  filter(multiverse_label == "plausible_within_filtered_0.5s_second test trial valid") 
#summarize pupil change cross full window by participant
participant_cross_window_summary_best_multiverse_variant <- best_multiverse_data |> 
  group_by(participant_lab_id,lab_id,condition, outcome, age_cohort) |>
  summarize(
    mean_pupil_z = mean(average_z, na.rm = TRUE)
  )

#summarize across participants
best_multiverse_variant.DESCRIPTIVES <- participant_cross_window_summary_best_multiverse_variant |>
  group_by(condition, outcome, age_cohort) |>
  summarize(
    N=n(),
    mean_pupil_change = mean(mean_pupil_z, na.rm=T), 
    sd_pupil_change = sd(mean_pupil_z, na.rm=T),
    ci_pupil_change = qt(1 - (0.05 / 2), N - 1) * (sd_pupil_change / sqrt(N)),
    lower_ci = mean_pupil_change - ci_pupil_change,
    upper_ci = mean_pupil_change + ci_pupil_change
      ) |>
  ungroup()
```
...
# PART 5 - Updated Multiverse LMER and LM Modeling Approach
...
ggplot(lmer_threewayinteraction_multiverse_results,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
...
#ICIS26
range(lmer_conditionXoutcome_multiverse_results_toddlers$estimate)
...
#ICIS26
range(lmer_conditionXoutcome_multiverse_results_adults$estimate)
...
fit_lm_main <- function(df,groups_to_include = c("adults","toddlers")) {
  ...
  if (is.data.frame(df) && n_distinct(df$condition_c) > 1 & 
    ...
    if (min_age_cohort_num == 1) {
      #Fit the model if conditions are met
      m <- lm(Average ~ condition_c * outcome_c, data = df.short)
    } else if (min_age_cohort_num == 2) {
      #Fit the model if conditions are met
...
# PART 6: Correlation between anticipatory looking and pupil dilation

These analyses correspond to main question 3 in the preregistration: Does increased pupil dilation in response to goal-incongruent events reflect a surprise response above and beyond it reflecting a novelty response? 
...
(no within-lab averaging)

```{r}
#summarize by lab within each multiverse dataset
summarize_participant_data_by_dataset_id <- data_for_plotting_df5 |> 
    group_by(multiverse_label, participant_lab_id, lab_id, condition, outcome, age_cohort) |>  
  summarize(
    mean_pupil_z = mean(average_z, na.rm = TRUE),
    prop_exit = prop_exit[1]
  ) |> 
  unite("multiverse_id_wcond", multiverse_label,condition, outcome, age_cohort, sep = "_", remove = FALSE)
```
...
#### Plot to show effect for each multiverse dataset,no within lab averaging

```{r}
#MV plot
summarize_participant_data_by_dataset_id %>%
  ggplot(aes(prop_exit,y=mean_pupil_z))+
  geom_smooth(aes(group=multiverse_id_wcond,color=outcome),alpha=0.05,method="lm",se=F)+
  facet_wrap(condition ~ age_cohort)+
  scale_color_ptol() +
  scale_fill_ptol()+
  facet_wrap(~ age_cohort+condition)+
  labs(x = "Anticipatory Looking",
           y = "Pupil size (z-score, relative change)")

ggsave(filename = here(plot_output_path,"mv_al_pd_vis.png"),width = 10, height = 6, dpi = 300)
```

```{r}
#"best" variant
#filter (node 1), filter (node 2), filter (node 3), baseline-correction 0.5s (node 4) and valid second test trial only (node 5)
best_multiverse_variant_participant_data <- summarize_participant_data_by_dataset_id |> 
  filter(multiverse_label == "plausible_within_filtered_0.5s_second test trial valid") 

ggplot(summarize_participant_data_by_dataset_id,aes(x=prop_exit,y=mean_pupil_z,color=outcome))+
  #geom_point(alpha=0.05)+
  geom_smooth(data=best_multiverse_variant_participant_data,aes(x=prop_exit,y=mean_pupil_z),size=1.2,method="lm")+
  facet_wrap(~ age_cohort+condition)+
  scale_color_ptol() +
  scale_fill_ptol()+
  geom_hline(yintercept = 0) +
...
ggsave(filename = here(plot_output_path,"mv_al_pd_vis_best_variant.png"),width = 10, height = 6, dpi = 300)

ggplot(summarize_participant_data_by_dataset_id,aes(x=prop_exit,y=mean_pupil_z,color=outcome))+
  geom_smooth(aes(group=multiverse_id_wcond,color=outcome,fill=outcome),method="lm",se=F)+
  geom_smooth(data=best_multiverse_variant_participant_data,aes(x=prop_exit,y=mean_pupil_z),size=1.2,method="lm")+
  facet_wrap(~ age_cohort+condition)+
  scale_color_ptol() +
  scale_fill_ptol()+
...
ggsave(filename = here(plot_output_path,"mv_al_pd_vis_final_combination_overlay.png"),width = 10, height = 6, dpi = 300)
```
...
```{r}
#function for fitting lm model for anticipatory looking and pupil dilation analyses
## this function automatically expands the interactions included depending on the number of groups and conditions included
## (up to a four-way interaction between anticipatory looking, outcome, condition, and age cohort)
fit_al_pd_lm <- function(df,groups_to_include = c("adults","toddlers"), conditions_to_include = c("knowledge","ignorance")) {
  if (length(groups_to_include)>1) {
    print("Fitting model across groups")
    min_age_cohort_num = 2
  } else {
    print("Fitting group-specific model")
    min_age_cohort_num = 1
  }
  if (length(conditions_to_include)>1) {
    print("Fitting model across conditions")
    min_condition_num = 2
  } else {
    print("Fitting condition-specific model")
    min_condition_num = 1
  }
  if (is.data.frame(df) && n_distinct(df$condition_c) >= min_condition_num & 
      n_distinct(df$outcome_c) > 1 & n_distinct(df$age_cohort_c) >= min_age_cohort_num) {
    ...
    df.short <- df |>
      ...
      #filter data to group to include
      filter(age_cohort %in% groups_to_include) |> 
      #filter data to conditions to include
      filter(condition %in% conditions_to_include) |> 
      group_by(participant_lab_id, condition_c, outcome_c, age_cohort,age_cohort_c, lab_id,prop_exit) |>
      summarize(Average = mean(average_z, na.rm=T)) |>
      ungroup() |> 
      mutate(
        age_cohort_adults = ifelse(age_cohort=="adults",0,1),
      ...
      # center prop_exit/ anticipatory looking
      mutate(
        prop_exit_c = prop_exit - mean(prop_exit, na.rm=TRUE)
      )
      
    if (min_age_cohort_num ==1) {
      if (min_condition_num ==1) {
        #Fit the model for a specific condition (2-way interaction: outcome * AL)
        m <- lm(Average ~ outcome_c * prop_exit_c, data = df.short)
      } else if (min_condition_num ==2) {
        #Fit the model for both conditions (3-way interaction: outcome * AL * condition)
        m <- lm(Average ~ outcome_c * prop_exit_c * condition_c, data = df.short)
      }
    } else if (min_age_cohort_num == 2) {
      if (min_condition_num ==1) {
        #Fit the model for a specific condition (3-way interaction: outcome * AL * age cohort)
        m <- lm(Average ~ outcome_c * prop_exit_c * age_cohort_c, data = df.short)
      } else if (min_condition_num ==2) {
        #Fit the model for both conditions (4-way interaction: outcome * AL * age cohort * condition)
        m <- lm(Average ~ outcome_c * prop_exit_c * age_cohort_c * condition_c, data = df.short)
      }
    }
    m
  } else {
...
Our main analysis focuses specifically on group-wise results (separately for toddlers and adults) in the knowledge condition.
...
```{r, warning=F, message=F}
#fit the lm models
models_al_pd_lm_knowledge_only_toddlers <- data_df5 %>%
  mutate(
    models = pmap(
      list(data,c("toddlers"),c("knowledge")),
      fit_al_pd_lm
    )
  ) %>%
  dplyr::select(-data) 
...
#add results
models_al_pd_lm_knowledge_only_w_results_toddlers <- models_al_pd_lm_knowledge_only_toddlers %>%
  mutate(
    results=map(models,extract_lm_model_results_tidy)
  )

#extract results into data frame
results_al_pd_lm_knowledge_only_toddlers <- models_al_pd_lm_knowledge_only_w_results_toddlers %>%
  dplyr::select(-models) %>%
  unnest(results) 
```
...
```{r}
#save the output
save(models_al_pd_lm_knowledge_only_w_results_toddlers, file = here(model_output_path, "pupillometry_models_al_pd_lm_knowledge_only_w_results_toddlers.Rds"))
save(results_al_pd_lm_knowledge_only_toddlers, file = here(model_output_path, "pupillometry_al_pd_lm_knowledge_only_toddlers.Rds"))
```

##### Plot Outcome by Anticipatory Looking Interaction

```{r}
al_pd_lm_knowledge_only_anticipationXoutcome_multiverse_results_toddlers <-results_al_pd_lm_knowledge_only_toddlers %>%
  filter(term=="outcome_c:prop_exit_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lm_knowledge_only_anticipationXoutcome_multiverse_results_toddlers,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
  xlab("Baseline Correction Window")+
  ylab("TODDLERS\nLm Model Estimate for 2-way interaction Outcome X AL in Knowledge Condition")

ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lm_knowledge_only_anticipationXoutcome_toddlers.png"),width = 10, height = 6, dpi = 300)
```

##### Plot Anticipatory Looking Effect

```{r}
al_pd_lm_knowledge_only_anticipation_multiverse_results_toddlers <-results_al_pd_lm_knowledge_only_toddlers %>%
  filter(term=="prop_exit_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lm_knowledge_only_anticipation_multiverse_results_toddlers,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
  xlab("Baseline Correction Window")+
  ylab("TODDLERS\nLm Model Estimate for Effect of Anticipatory Looking in Knowledge Condition")

ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lm_knowledge_only_anticipation_toddlers.png"),width = 10, height = 6, dpi = 300)
```

##### Plot Outcome Effect

```{r}
al_pd_lm_knowledge_only_outcome_multiverse_results_toddlers <-results_al_pd_lm_knowledge_only_toddlers %>%
  filter(term=="outcome_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lm_knowledge_only_outcome_multiverse_results_toddlers,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
  xlab("Baseline Correction Window")+
  ylab("TODDLERS\nLm Model Estimate for Effect of Outcome in Knowledge Condition")

ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lm_knowledge_only_outcome_toddlers.png"),width = 10, height = 6, dpi = 300)
```
...
```{r, warning=F, message=F}
#fit the lm models
models_al_pd_lm_knowledge_only_adults <- data_df5 %>%
  mutate(
    models = pmap(
      list(data,c("adults"),c("knowledge")),
      fit_al_pd_lm
    )
  ) %>%
  dplyr::select(-data) 
...
#add results
models_al_pd_lm_knowledge_only_w_results_adults <- models_al_pd_lm_knowledge_only_adults %>%
  mutate(
    results=map(models,extract_lm_model_results_tidy)
  )

#extract results into data frame
results_al_pd_lm_knowledge_only_adults <- models_al_pd_lm_knowledge_only_w_results_adults %>%
  dplyr::select(-models) %>%
  unnest(results) 
```
...
```{r}
#save the output
save(models_al_pd_lm_knowledge_only_w_results_adults, file = here(model_output_path, "pupillometry_models_al_pd_lm_knowledge_only_w_results_adults.Rds"))
save(results_al_pd_lm_knowledge_only_adults, file = here(model_output_path, "pupillometry_al_pd_lm_knowledge_only_adults.Rds"))
```

##### Plot Outcome by Anticipatory Looking Interaction

```{r}
al_pd_lm_knowledge_only_anticipationXoutcome_multiverse_results_adults <-results_al_pd_lm_knowledge_only_adults %>%
  filter(term=="outcome_c:prop_exit_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lm_knowledge_only_anticipationXoutcome_multiverse_results_adults,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
...
ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lm_knowledge_only_anticipationXoutcome_adults.png"),width = 10, height = 6, dpi = 300)
```

##### Plot Anticiptory Looking Effect

```{r}
al_pd_lm_knowledge_only_anticipation_multiverse_results_adults <-results_al_pd_lm_knowledge_only_adults %>%
  filter(term=="prop_exit_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lm_knowledge_only_anticipation_multiverse_results_adults,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
...
ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lm_knowledge_only_anticipation_adults.png"),width = 10, height = 6, dpi = 300)
```

##### Plot Outcome Effect

```{r}
al_pd_lm_knowledge_only_outcome_multiverse_results_adults <-results_al_pd_lm_knowledge_only_adults %>%
  filter(term=="outcome_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lm_knowledge_only_outcome_multiverse_results_adults,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
...
ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lm_knowledge_only_outcome_adults.png"),width = 10, height = 6, dpi = 300)
```
...
```{r}
#function for fitting lm model for anticipatory looking and pupil dilation analyses
## this function automatically expands the interactions included depending on the number of groups
## (up to a three-way interaction between anticipatory looking, outcome, and age cohort)
## restricted to only using one condition for now (rather than full four-way interaction)
run_al_pd_lmer_re_pruning <- function(df,groups_to_include = c("adults","toddlers"),condition_to_include = "knowledge") {
  if (length(groups_to_include)>1) {
    print("Fitting model across groups")
    min_age_cohort_num = 2
    lmer_model_type = "main"
  ...
  if (condition_to_include=="knowledge") {
    print("Fitting model for Knowledge condition")
  } else if (condition_to_include == "ignorance") {
    print("Fitting model for Ignorance condition")
  } else {
    print("condition name not recognized. Unexpected condition name:")
    print(toString(condition_to_include))
    # Return NULL if condition parameter has unexpected value
    NULL
  }
  if (is.data.frame(df) && 
      n_distinct(df$outcome_c) > 1 & n_distinct(df$age_cohort_c) >= min_age_cohort_num) {
    ...
    df.short <- df |>
      ...
      filter(age_cohort %in% groups_to_include) |> 
      #filter data to conditions to include
      filter(condition == condition_to_include) |> 
      group_by(participant_lab_id, condition_c, outcome_c, age_cohort,age_cohort_c, lab_id,prop_exit) |>
      summarize(Average = mean(average_z, na.rm=T)) |>
      ungroup() |> 
      mutate(
    ...
    #Fit the model if conditions are met
    prune_al_lmer_model(df.short,model_type=lmer_model_type)
  } else {
    # Return NULL if conditions aren't met
    NULL
  }
...
```{r, warning=F, message=F}
#fit the lmer models
models_al_pd_lmer_knowledge_only_toddlers <- data_df5 %>%
  mutate(
    models = pmap(
      list(data,c("toddlers"),"knowledge"),
      run_al_pd_lmer_re_pruning
    )
  ) %>%
  dplyr::select(-data) 

#add results
models_al_pd_lmer_knowledge_only_w_results_toddlers <- models_al_pd_lmer_knowledge_only_toddlers %>%
  mutate(
    results=map(models,extract_lmer_model_results_tidy)
  )

#extract results into data frame
results_al_pd_lmer_knowledge_only_toddlers <- models_al_pd_lmer_knowledge_only_w_results_toddlers %>%
  dplyr::select(-models) %>%
  unnest(results) 
```
...
```{r}
#save the output
save(models_al_pd_lmer_knowledge_only_w_results_toddlers, file = here(model_output_path, "pupillometry_models_al_pd_lmer_knowledge_only_w_results_toddlers.Rds"))
save(results_al_pd_lmer_knowledge_only_toddlers, file = here(model_output_path, "pupillometry_results_al_pd_lmer_knowledge_only_toddlers.Rds"))
```

##### Plot Outcome by Anticipatory Looking Interaction

```{r}
al_pd_lmer_knowledge_only_anticipationXoutcome_multiverse_results_toddlers <-results_al_pd_lmer_knowledge_only_toddlers %>%
  filter(term=="outcome_c:prop_exit_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lmer_knowledge_only_anticipationXoutcome_multiverse_results_toddlers,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
  xlab("Baseline Correction Window")+
  ylab("TODDLERS\nLMER Model Estimate for 2-way interaction Outcome X AL in Knowledge Condition")

ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lmer_knowledge_only_anticipationXoutcome_toddlers.png"),width = 10, height = 6, dpi = 300)
```

##### Plot Anticipatory Looking Effect

```{r}
al_pd_lmer_knowledge_only_anticipation_multiverse_results_toddlers <-results_al_pd_lmer_knowledge_only_toddlers %>%
  filter(term=="prop_exit_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lmer_knowledge_only_anticipation_multiverse_results_toddlers,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
  xlab("Baseline Correction Window")+
  ylab("TODDLERS\nLMER Model Estimate for Effect of Anticipatory Looking in Knowledge Condition")

ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lmer_knowledge_only_anticipation_toddlers.png"),width = 10, height = 6, dpi = 300)
```

##### Plot Outcome Effect

```{r}
al_pd_lmer_knowledge_only_outcome_multiverse_results_toddlers <-results_al_pd_lmer_knowledge_only_toddlers %>%
  filter(term=="outcome_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lmer_knowledge_only_outcome_multiverse_results_toddlers,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
  xlab("Baseline Correction Window")+
  ylab("TODDLERS\nLMER Model Estimate for Effect of Outcome in Knowledge Condition")

ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lmer_knowledge_only_outcome_toddlers.png"),width = 10, height = 6, dpi = 300)
```
...
```{r, warning=F, message=F}
#fit the lmer models
models_al_pd_lmer_knowledge_only_adults <- data_df5 %>%
  mutate(
    models = pmap(
      list(data,c("adults"),"knowledge"),
      run_al_pd_lmer_re_pruning
    )
  ) %>%
  dplyr::select(-data) 
...
#add results
models_al_pd_lmer_knowledge_only_w_results_adults <- models_al_pd_lmer_knowledge_only_adults %>%
  mutate(
    results=map(models,extract_lmer_model_results_tidy)
  )

#extract results into data frame
results_al_pd_lmer_knowledge_only_adults <- models_al_pd_lmer_knowledge_only_w_results_adults %>%
  dplyr::select(-models) %>%
  unnest(results) 
```
...
```{r}
#save the output
save(models_al_pd_lmer_knowledge_only_w_results_adults, file = here(model_output_path, "pupillometry_models_al_pd_lmer_knowledge_only_w_results_adults.Rds"))
save(results_al_pd_lmer_knowledge_only_adults, file = here(model_output_path, "pupillometry_results_al_pd_lmer_knowledge_only_adults.Rds"))
```

##### Plot Outcome by Anticipatory Looking Interaction

```{r}
al_pd_lmer_knowledge_only_anticipationXoutcome_multiverse_results_adults <-results_al_pd_lmer_knowledge_only_adults %>%
  filter(term=="outcome_c:prop_exit_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lmer_knowledge_only_anticipationXoutcome_multiverse_results_adults,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
...
ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lmer_knowledge_only_anticipationXoutcome_adults.png"),width = 10, height = 6, dpi = 300)
```

##### Plot Anticipatory Looking Effect

```{r}
al_pd_lmer_knowledge_only_anticipation_multiverse_results_adults <-results_al_pd_lmer_knowledge_only_adults %>%
  filter(term=="prop_exit_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lmer_knowledge_only_anticipation_multiverse_results_adults,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
...
ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lmer_knowledge_only_anticipation_adults.png"),width = 10, height = 6, dpi = 300)
```

##### Plot Outcome Effect

```{r}
al_pd_lmer_knowledge_only_outcome_multiverse_results_adults <-results_al_pd_lmer_knowledge_only_adults %>%
  filter(term=="outcome_c") %>%
  mutate(
    significant = ifelse(p.value<0.05,"significant","non-significant")
  )
ggplot(al_pd_lmer_knowledge_only_outcome_multiverse_results_adults,aes(x=df4_baseline_correction,y=estimate,alpha=significant))+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_errorbar(aes(ymin=conf.low,ymax=conf.high),width=0)+
  geom_point(size=2)+
  facet_grid(df1_extreme_values+df2_screen_fixation ~ df3_moving_average+df5_ppt_exclusion)+
...
ggsave(filename = here(plot_output_path,"mv_effect_plot_al_pd_lmer_knowledge_only_outcome_adults.png"),width = 10, height = 6, dpi = 300)
```

## Exploratory

Here, I'm just using the best-variant fit for now to explore the three-way interaction that includes condition.

```{r}
best_multiverse_variant_participant_data <- best_multiverse_variant_participant_data |> 
  ungroup() |> 
mutate(
    age_cohort_c = case_when(
      age_cohort == "adults" ~ -0.5,
...
### Three-Way Interaction: Outcome * Anticipatory Looking * Condition

#### Toddlers

```{r}
#fit lmer model
## including random slopes for outcome seems to not allow for convergence
m_al_3wayint_toddlers <- lmer(mean_pupil_z ~ outcome_c * prop_exit_c * condition_c+(1+ prop_exit_c*condition_c|lab_id), data = filter(best_multiverse_variant_participant_data, age_cohort=="toddlers"))
summary(m_al_3wayint_toddlers)
```
...
```{r}
#fit lmer model
## including random slopes for outcome or condition seems to not allow for convergence
m_al_3wayint_adults <- lmer(mean_pupil_z ~ outcome_c * prop_exit_c * condition_c+(1+ prop_exit_c|lab_id), data = filter(best_multiverse_variant_participant_data, age_cohort=="adults"))
summary(m_al_3wayint_adults)
```

### Effect of anticipation in on pupil size in incongruent, knowledge condition specifically?

#### Toddlers

```{r}
m_al_incongruent_knowledge_toddlers <- lmer(
  mean_pupil_z ~ prop_exit_c +(1+ prop_exit_c|lab_id),
  filter(best_multiverse_variant_participant_data, age_cohort=="toddlers"& condition=="knowledge" & outcome=="incongruent"))
summary(m_al_incongruent_knowledge_toddlers)
```

#### Adults

```{r}
#issues with convergence of lmer model
# m_al_incongruent_knowledge_adults <- lmer(
#   mean_pupil_z ~ prop_exit_c +(1|lab_id),
#   filter(best_multiverse_variant_participant_data, age_cohort=="adults"& condition=="knowledge" & outcome=="incongruent"))
m_al_incongruent_knowledge_adults <- lm(
  mean_pupil_z ~ prop_exit,
  filter(best_multiverse_variant_participant_data, age_cohort=="adults"& condition=="knowledge" & outcome=="incongruent"))
summary(m_al_incongruent_knowledge_adults)
```