...
# For the subsequent step, i.e., first degree, identify those z-values that correspond to 2mm and 8mm.
data_pupillometry |> 
  filter(eyetracker_type != "EyeLink", average <= 2 ) |>
  filter(average == max(average, na.rm = T)) |>
  pull(average_z) # 2mm corresponds to  z = -2.445436
#
data_pupillometry |> 
  filter(eyetracker_type != "EyeLink", average >= 7.18) |>
  filter(average == max(average, na.rm = T)) |>
  pull(average_z) # 6.5mm corresponds to  z = 5.043629

# Are there any values greater than z = 5.043629?
data_pupillometry |> 
  filter(average_z >= 5.04)
...
data_pupillometry <- data_pupillometry |> 
  mutate(average = ifelse(pupil_left < 0, NA, average),
         average = ifelse(average < 0, NA, average),
         average_z = ifelse(pupil_left < 0, NA, average_z),
         average_z = ifelse(pupil_right < 0, NA, average_z),
         )

#! This needs to be done on a subject by recording level, i.e., for each Tobii-participant determine the z-value that corresponds to 2mm and 8mm. Then determine to 'largest' 2mm z-score and the 'smallest' 8mm z-score across all Tobii-subjects. Those should be the cutoffs.

data_plausible <- data_pupillometry |> 
    mutate(average = ifelse(average_z < -2.45 | average_z > 5.04, NA, average)) |>
    mutate(average_z = ifelse(average_z < -2.45 | average_z > 5.04, NA, average_z))
...
# Here plot with z-values
data_pupillometry |> 
  mutate(plausibility = ifelse(pupil_left_z <= -2.45 | pupil_left_z >= 5.04 | 
                                 pupil_right_z <= -2.45 | pupil_right_z >= 5.04,
                               "implausible", "plausible")) |> 
  ggplot(aes(x = pupil_left_z, y = pupil_right_z, col = plausibility)) +
  geom_point(alpha = .1) +
  labs(x = "Pupil size left eye (z-score)",
       y = "Pupil size right eye (z-score)") +
  scale_color_manual(values = c("indianred", "royalblue")) +
  facet_wrap("age_cohort")
```
...
data_avg <- data_df2 |> 
  mutate(data = map(data, \(d) { 
    d |> 
      ...
      #                                fill = NA, align = "right")) |> 
      # rowwise() |> 
      # mutate(average = mean(c(pupil_left, pupil_right), na.rm = TRUE)) |> 
      ungroup() 
  }))
...
data_df3 |> 
  pull(data) |> 
  (`[`)(value = c(1,1)) |> # arbitrarily select two universes to visualise
  map(\(d) {
...
baseline_correct <- function(data, TIME_WINDOW = 1000) {
  data |>
    ...
    mutate(average_z = {
      avg_filtered_z <- average_z[t_norm >= -TIME_WINDOW & t_norm < 0]
      avg_mean_z <- mean(avg_filtered_z, na.rm = TRUE)
      (average_z - avg_mean_z)/avg_mean_z # Changed to calculate relative change.
    })
}
...
data_df4 |> 
  pull(data) |> 
  (`[`)(value = c(1,1)) |> # arbitrarily select three universes to visualise
  map(\(d) {
    ggplot(d |> filter(!is.na(average_z),
                       t_norm >= 0 & t_norm < 5000), 
           aes(x = t_norm, y = average_z, col = outcome, linetype = age_cohort)) +
...
data_df4 |> 
  pull(data) |> 
  (`[`)(value = c(3,1)) |> # arbitrarily select two universes to visualise
  map(\(d) {
