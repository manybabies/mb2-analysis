---
title: "MB2-P - Multiverse preprocessing and linear modeling"
author: "Rmarkdown by Giulia Calignano, Melli Schreiner, Alvin Tan, Robert Hepach "
date: "`r Sys.Date()`"
output:
  html_document:
    df_print: paged
...
model_results <- map(data_df5_list, run_model)
...
model_metrics <- map(model_results, extract_model_metrics)
model_metrics <- compact(model_metrics)  # Remove any NULL elements

# Convert to a tidy dataframe
model_metrics_df <- bind_rows(model_metrics, .id = "model_id")

# Check the structure of model_metrics_df to confirm columns are present
str(model_metrics_df)
...
# Create the plot
ggplot(model_metrics_df, aes(x = model_order, y = BIC, color = model_id)) +
...
ggplot(model_metrics_df, aes(x = model_order, y = AIC, color = model_id)) +
  geom_point() +                                # Points for estimates
...
ggplot(model_metrics_df, aes(x = model_order, y = R_squared, color = model_id)) +
  geom_point() +                                # Points for estimates
...
```{r interaction plot, echo=TRUE, include=TRUE, out.width='90%'}
# Load the sjPlot package for plotting
library(sjPlot)
library(purrr)

# Check if each model has interaction terms and plot them
plot_list <- map(model_results, function(model) {
  if (!is.null(model)) {
    # Plot interaction terms for each model
    plot <- plot_model(model, type = "int", terms = c("condition", "outcome", "age_cohort"))
    return(plot)
  }
})

# Display the plots
# plot_list will contain all the individual plots
plot_list
