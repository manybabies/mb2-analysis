...
data_pupillometry |>
  filter(average > 1.8) |>
  select(lab_id, average, eyetracker_type)
...
# ICIS26
# Get descriptives
best_multiverse_variant.DESCRIPTIVES <- best_multiverse_variant |>
  group_by(condition, outcome, age_cohort) |>
  summarize(MEANPerPart = mean(mean_pupil_z, na.rm=T), SDPerPart = sd(mean_pupil_z, na.rm=T)) |>
  ungroup()
```
