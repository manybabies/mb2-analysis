...
- **conditions**: A catehorical variable with two levels, i.e. "knowledge" and "ignorance"
- **outcomes**: A categorical variable with two levels, i.e. "same" and "empty"
...
all$participant_id = as.factor(all$participant_id)
all$lab_id = as.factor(all$lab_id)
all$age_cohort = as.factor(all$lab_id)
...
### i.e. 5s before resulution vs 300 abd 500 ms after bear resolution
library(plyr)
...
As a final step, we investigated (1) the averaged interaction effect Conditions x Outcome, (2) the time-course of the interaction effect Conditions x Outcome and (3) the non linear interaction effect of Conditions x Outcome considering the time-course of the effect. This approach provided an exploration of whether and how smoothing time enhances the plausibility of statistical modeling of pupil dilation across the 54 datasets we created.
...
# Initialize a list to store the results for all datasets in the megaverse
megaverse_models <- list()

# Loop through each dataset in the megaverse and fit the models
for (i in seq_along(megaverse)) {
  dataset <- megaverse[[i]]
  models <- fit_models(dataset)
  megaverse_models[[i]] <- models
}

# megaverse_models now contains the results of the three models for each dataset

# Define a function to fit three different linear models to a dataset and create predictions
fit_and_predict <- function(data) {
  ...
  # Create a data frame with predictions for each model
  data$pred1 <- predict(model1, data)
  data$pred2 <- predict(model2, data)
  data$pred3 <- predict(model3, data)
  
  return(data)
}

# Initialize a list to store the datasets with predictions
megaverse_predictions <- list()

# Loop through each dataset in the megaverse and create predictions
for (i in seq_along(megaverse)) {
  dataset <- megaverse[[i]]
  dataset_with_predictions <- fit_and_predict(dataset)
  megaverse_predictions[[i]] <- dataset_with_predictions
}

# Define a function to plot the effects of the models
plot_effects <- function(data, dataset_name) {
  # Melt the data frame to long format for ggplot2
  library(reshape2)
  data_long <- melt(data, id.vars = c("x1", "x2", "y"), 
                    measure.vars = c("pred1", "pred2", "pred3"), 
                    variable.name = "model", value.name = "prediction")
  
  # Create the plot
  p <- ggplot(data_long, aes(x = y, y = prediction, color = model)) +
    geom_point(alpha = 0.5) +
    geom_smooth(method = "lm", se = FALSE) +
    labs(title = paste("Effects of Models on", dataset_name),
...
# Loop through each dataset with predictions and create plots
plots <- list()
for (i in seq_along(megaverse_predictions)) {
  dataset_with_predictions <- megaverse_predictions[[i]]
  dataset_name <- paste("Dataset", i)
  plot <- plot_effects(dataset_with_predictions, dataset_name)
  plots[[i]] <- plot
}
