...
# Define the function to fit models for anticipatory looking in the specified pruning order
prune_al_lmer_model <- function(data, model_type = "main",age_cohort_predictor_name="age_cohort_c") {
  if (model_type=="main") {
    # Shared part of the formula
    shared_formula <- paste0("Average ~ outcome_c * prop_exit_c * ",age_cohort_predictor_name)
  } else if (model_type=="group") {
    # Shared part of the formula
    shared_formula <- paste0("Average ~ outcome_c * prop_exit_c")
  }
