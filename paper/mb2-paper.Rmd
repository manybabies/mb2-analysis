...
# Extract results
e_bm_fam_aoi_toddlers <- tidy(bm_fam_aoi_toddlers, effects = "fixed")
e_bm_fam_aoi_adults <- tidy(bm_fam_aoi_adults, effects = "fixed")
e_bm_fam_first_look_toddlers <- tidy(bm_fam_first_look_toddlers, effects = "fixed")
e_bm_fam_first_look_adults <- tidy(bm_fam_first_look_adults, effects = "fixed")
e_bm_aoi_toddlers <- tidy(bm_aoi_toddlers, effects = "fixed")
e_bm_aoi_adults <- tidy(bm_aoi_adults, effects = "fixed")
e_bm_test_first_look_toddlers <- tidy(bm_test_first_look_toddlers, effects = "fixed")
e_bm_test_first_look_adults <- tidy(bm_test_first_look_adults, effects = "fixed")

# Add model names
e_bm_fam_aoi_toddlers <- e_bm_fam_aoi_toddlers %>% mutate(model = "PTL toddlers")
e_bm_fam_aoi_adults <- e_bm_fam_aoi_adults %>% mutate(model = "PTL adults")
e_bm_fam_first_look_toddlers <- e_bm_fam_first_look_toddlers %>% mutate(model = "First Look toddlers")
e_bm_fam_first_look_adults <-  e_bm_fam_first_look_adults %>% mutate(model = "First Look adults")
e_bm_aoi_toddlers <- e_bm_aoi_toddlers %>% mutate(model = "PTL toddlers")
e_bm_aoi_adults <- e_bm_aoi_adults %>% mutate(model = "PTL adults")
e_bm_test_first_look_toddlers <- e_bm_test_first_look_toddlers %>% mutate(model = "First Look toddlers")
e_bm_test_first_look_adults <- e_bm_test_first_look_adults %>% mutate(model = "First Look adults")
...
The Bayes factor comparing the full model to the simpler model was estimated to be `r format_bayes_factor(fam_m_comparison_FL_toddlers$bf)`, strongly favoring the full model over the null model.
...
In order to investigate whether there's an interaction of condition and test trial number, we fitted Bayesian mixed-effects model to predict PTL with fixed effects for condition, test trial number, and their interaction, along with random intercepts and slopes for these variables across labs, for toddlers and adults separately. While for toddlers, the results were inconclusive (`r format_bayes_factor(test_comparison_aoi_toddlers_both_order$bf)`), for adults, the Bayes Factor of `r format_bayes_factor(test_comparison_aoi_adults_both_order$bf)` provided strong evidence for including the interaction of condition and test trial number as fixed effect. Overall, the results demonstrate that while PTL increased over trials, this effect was moderated by the condition, with the ignorance condition showing a slower rate of increase compared to the knowledge condition.
...
# Extract summary for the specific fixed effect
fixed_effect_summary_shifts_age_cohort <- posterior_summary(bm_test_shifts_age_cohort, pars = "b_age_cohort_c")

# Extract the mean estimate
b_age_cohort_c_estimate_shifts <- fixed_effect_summary_shifts_age_cohort[1, "Estimate"]

custom_notation <- function(x) {
  if (x > 1000) {
    ">1000"
  } else {
    as.character(x)
