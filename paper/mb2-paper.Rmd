...
The Bayes factor comparing the base model to the simpler model was estimated to be `r printnum(fam_m_comparison_FL_toddlers$bf)`, strongly favoring the base model over the null model.
...
As first model, we used a Bayesian mixed effects models to predict PTL based on condition and age for toddlers, with random effects for lab and participants and random slopes for trial number for each. The Bayes factor comparing this model to the simpler null model without the intercept was estimated to be `r printnum(fam_m_comparison_PTL_toddlers$bf)`, strongly favoring the base model over the null model. Table 4 shows the statistics for regression coefficients of the base model. These results suggest a significant effect of condition on PTL, with the negative coefficient indicating a decrease in PTL across the familiarization trials. The strong Bayes factor provides substantial evidence in support of the base model, highlighting its robustness in explaining the data compared to the null model.
...
Investigating proportion of first looks for toddlers, we again used a Bayesian mixed effects model to predict target looks based on trial number (1-4), with random effects for lab and participants and random slopes for trial number for each. The Bayes factor comparing the base model to the simpler model was estimated to be `r printnum(fam_m_comparison_PTL_adults$bf)`, strongly favoring the base model over the null model. There was a significant effect of trial number on proportion of first looks, with the negative coefficient indicating a decrease in target looks across the familiarization trials. The strong Bayes factor provides substantial evidence in support of the base model, highlighting its robustness in explaining the data compared to the null model. Using binomal test, the proportion of first looks of `r filter(summary_fam_aoi_overall, age_cohort=="toddlers")$mean_target_looking` significantly differed from chance,
<!-- *t*(`r filter(summary_fam_aoi_overall, age_cohort=="toddlers")$df`) = `r filter(summary_fam_aoi_overall, age_cohort=="toddlers")$tval`, *p* `r printp(filter(summary_fam_aoi_overall, age_cohort=="toddlers")$p.value)`, suggesting that toddlers successfully anticipated during the familiarization trials. -->

##### Adults

Comparing the Bayesian mixed effects model of adults predicting proportion of first looks based on trial number (1-4), with random effects for lab and participants and random slopes for trial number for each with the simpler model without an intercept, we computed a Bayes factor of `r printnum(fam_m_comparison_PTL_adults$bf)`, strongly favoring the base model over the null model. There was a significant effect of trial number on proportion of first looks, with the positive coefficient indicating an increase in proportion of first target looks across the familiarization trials. The strong Bayes factor provides substantial evidence in support of the base model, highlighting its robustness in explaining the data compared to the null model. Using binomal test, the proportion of first looks of `r filter(summary_fam_aoi_overall, age_cohort=="toddlers")$mean_target_looking` significantly differed from chance,
<!-- *t*(`r filter(summary_fam_aoi_overall, age_cohort=="toddlers")$df`) = `r filter(summary_fam_aoi_overall, age_cohort=="toddlers")$tval`, *p* `r printp(filter(summary_fam_aoi_overall, age_cohort=="toddlers")$p.value)`, suggesting that toddlers successfully anticipated during the familiarization trials. -->
...
#### Whether effects are magnified or diminished on the second trial (order main effect), and (3) whether there is evidence of “spillover” - dependency in anticipation on the second trial depending on what the first trial is (condition x order interaction effect)
...
#### Comparing the looking patterns of toddlers and adults during the location change of the mouse.
We fitted a Bayesian mixed-effects model to examine the relationship between the number of shifts between mouse and bear and age cohort during location change of the mouse, while accounting for random effects by lab_id. The fixed effect of age cohort on number of shifts is estimated to be `r round(b_num_shifts_estimate_shifts, 3)`, suggesting a higher number of shifts for adults compared to toddlers (see Figure 6). Comparing this model to a simpler model without age cohort, a Bayes Factor of `r printnum(test_m_comparison_shifts_al$bf)` was computed. This provides extremely strong evidence in favor of including age cohort in the model.

```{r fig6, fig.cap="Number of shifts between mouse and bear during location change of mouse in the test phase for toddlers and adults.", fig.align="center", out.width = "6.8in"}
include_graphics(here("paper","Figure6.png"),rel_path=FALSE)
```



#### Number of gaze shifts between mouse and bear during location change as a function of AL.
In order to examine the effect of condition and the number of shifts between mouse and bear during location change of the mouse on anticipatory looking, we fitted a Bayesian mixed-effects model. The dependent variable was the proportion of target looking. The fixed effects included the main effects of condition, the number of shifts, and their interaction. We also included random intercepts and slopes for number of shifts within each participant and within each lab, allowing us to account for the hierarchical structure of the data and potential variability between participants. The fixed effect of difference in mouse-bear looking on number of shifts is estimated to be `r round(b_age_cohort_c_estimate_shifts, 3)`, suggesting a higher number of shifts for adults compared to toddlers (see Figure 6). Comparing this model to a simpler model without age cohort, a Bayes Factor of `r printnum(test_m_comparison_shifts_age_cohort$bf)` was computed. This provides extremely strong evidence in favor of including age cohort in the model.

```{r fig7, fig.cap="Number of shifts between mouse and bear during location change of mouse as a function of AL in the test phase for toddlers and adults.", fig.align="center", out.width = "6.8in"}
...
# Extract summary for the specific fixed effect
fixed_effect_summary_diff_mouse_bear <- posterior_summary(bm_test_shifts_age_cohort, pars = "b_diff_mouse_bear")
...
### Latency of first look to target AOI (longer latency as indicator of uncertainty)
Hypothesis: Longer latency  in KNOW vs. IG

### Absolute looking scores
