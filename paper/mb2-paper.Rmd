...
load(here(RESULTS_FOLDER,"summary_fam_aoi_overall.rds"))
load(here(RESULTS_FOLDER,"bm_fam_aoi_toddlers.rds"))
load(here(RESULTS_FOLDER,"bm_fam_aoi_adults.rds"))
load(here(RESULTS_FOLDER,"fam_m_comparison_PTL_toddlers.rds"))
load(here(RESULTS_FOLDER,"fam_m_comparison_PTL_adults.rds"))
load(here(RESULTS_FOLDER,"fam_m_comparison_FL_toddlers.rds"))
load(here(RESULTS_FOLDER,"fam_m_comparison_FL_adults.rds"))
```
...
As first model, we used a Bayesian mixed effects models to predict PTL based on trial number (1-4) for toddlers, with random effects for lab and participants and random slopes for trial number for each. The Bayes factor comparing this model to the simpler null model without the intercept was estimated to be `r round(fam_m_comparison_PTL_toddlers$bf, 0)`, strongly favoring the base model over the null model. See also Table X for regression coefficients for the base and the null model. These results suggest a significant effect of trial number on PTL, with the negative coefficient indicating a decrease in PTL across the familiarization trials. The strong Bayes factor provides substantial evidence in support of the base model, highlighting its robustness in explaining the data compared to the null model.
In addition, toddlers' mean PTL of `r filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$mean_target_looking` significantly differed from chance, *t*(`r filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$df`) = `r filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$tval`, *p* = `r format(filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$p.value, scientific = TRUE, digits = 3)`, suggesting that toddlers successfully anticipated during the familiarization trials.

##### Adults

Next, we used a Bayesian mixed effects model to predict PTL based on trial number (1-4) for adults, again with random effects for lab and participants and random slopes for trial number for each. The Bayes factor comparing this model to the simpler null model without the intercept was estimated to be `r round(fam_m_comparison_PTL_adults$bf, 0)`, strongly favoring the base model over the null model. These results suggest a significant effect of trial number on PTL, with the negative coefficient indicating an increase in target looks across the familiarization trials. The strong Bayes factor provides substantial evidence in support of the base model, highlighting its robustness in explaining the data compared to the null model. In addition, adults' mean PTL of `r filter(summarize_fam_aoi_overall, age_cohort=="adults")$mean_target_looking` significantly differed from chance, *t*(`r filter(summarize_fam_aoi_overall, age_cohort=="adults")$df`) = `r filter(summarize_fam_aoi_overall, age_cohort=="adults")$tval`, *p* = `r format(filter(summarize_fam_aoi_overall, age_cohort=="adults")$p.value, scientific = TRUE, digits = 3)`, suggesting that adults successfully anticipated during the familiarization trials.

#### Proportion of first looks

##### Toddlers

Investigating proportion of first looks for toddlers, we again used a Bayesian mixed effects model to predict target looks based on trial number (1-4), with random effects for lab and participants and random slopes for trial number for each. The Bayes factor comparing the base model to the simpler model was estimated to be `r round(fam_m_comparison_PTL_adults$bf, 0)`, strongly favoring the base model over the null model. There was a significant effect of trial number on proportion of first looks, with the negative coefficient indicating a decrease in target looks across the familiarization trials. The strong Bayes factor provides substantial evidence in support of the base model, highlighting its robustness in explaining the data compared to the null model. Using binomal test, the proportion of first looks of `r filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$mean_target_looking` significantly differed from chance, *t*(`r filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$df`) = `r filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$tval`, *p* = `r round(filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$p.value, 3)`, suggesting that toddlers successfully anticipated during the familiarization trials.

##### Adults

Comparing the Bayesian mixed effects model of adults predicting proportion of first looks based on trial number (1-4), with random effects for lab and participants and random slopes for trial number for each with the simpler model without an intercept, we computed a Bayes factor of `r round(fam_m_comparison_PTL_adults$bf, 0)`, strongly favoring the base model over the null model. There was a significant effect of trial number on proportion of first looks, with the positive coefficient indicating an increase in proportion of first target looks across the familiarization trials. The strong Bayes factor provides substantial evidence in support of the base model, highlighting its robustness in explaining the data compared to the null model. Using binomal test, the proportion of first looks of `r filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$mean_target_looking` significantly differed from chance, *t*(`r filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$df`) = `r filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$tval`, *p* = `r round(filter(summarize_fam_aoi_overall, age_cohort=="toddlers")$p.value, 3)`, suggesting that toddlers successfully anticipated during the familiarization trials.
