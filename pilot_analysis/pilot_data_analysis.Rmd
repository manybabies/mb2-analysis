...
d <- labs %>%
  map_df(function(lab) {
    ...
    left_join(aoi_data, subjects) %>%
      ...
      mutate(window_aoi  = grepl("window", aoi),
          aoi = case_when(grepl("target", aoi) ~ "target",
...
filter(ungroup(d), t > -4120, t < -3900) %>%
  group_by(t) %>%
  summarise(m=mean(aoi != "other"))
...
```{r}
maxt = group_by(d, subid, trial_id, lab) %>%
  summarise(n=n()) %>%
  ungroup() %>%
  mutate(maxn = max(n),
         newn=n/maxn)
...
ggplot(rename(ms, AOI = region) %>%
         mutate(e = case_when(experiment_num == "pilot_1a" ~ "Pilot 1 Kids",
                                           experiment_num == "pilot_1a_adult" ~ "Pilot 1 Adults",
                                           experiment_num == "pilot_1b_no_outcome" ~ "Pilot 2 (no outcome",
                experiment_num == "pilot_1b_outcome" ~ "Pilot 2 (outcome)")),
       aes(x = t, y = looking, col = AOI)) + 
  geom_line() + 
  ...
  xlab("time (ms)") + ylab("proportion looking") #only consider the 4sec window before and after POD
ggsave("supp_fig1.png")
...
ggplot(mutate(ms, e = case_when(experiment_num == "pilot_1a" ~ "Pilot 1 Kids",
                                           experiment_num == "pilot_1a_adult" ~ "Pilot 1 Adults",
                                           experiment_num == "pilot_1b_no_outcome" ~ "Pilot 2",
                experiment_num == "pilot_1b_outcome" ~ "Pilot 2")), aes(x = t, y = looking, col = region)) +
  geom_point() + 
...
ggsave("across_trial_blocks.png")
```
```{r}
ms <- d %>%
  mutate(block = ifelse(trial_num < 5, "Trials 1-4", "Trials 5-8")) %>%
  ...
  summarise(target = mean(aoi == "target", na.rm = TRUE),
            distractor = mean(aoi == "distractor", na.rm = TRUE),
            window = mean(window_aoi == T, na.rm=T)) %>%
  gather(region, looking, window) 
...
ggplot(mutate(ms, e = case_when(experiment_num == "pilot_1a" ~ "Pilot 1 Kids",
  ...
  theme(legend.position = "bottom") + 
  facet_grid(e~block)
ggsave("looks_to_window.png", width=5, height=5)
```
...
group_by(first_looks_df, experiment_num) %>%
  summarise(nlabs=length(unique(lab)),
            aoi = mean(aoi == "target"))

group_by(first_looks_df, grepl("adult", experiment_num)) %>%
  summarise(nlabs=length(unique(lab)),
            aoi = mean(aoi == "target"))
```
...
p1a_first_look <- ggplot(data = filter(first_looks_df, experiment_num == "pilot_1a")) + 
  ...
  labs(title = "Pilot 1 (Children) First Look") +
  ylab("Proportion") 

ggsave(filename = "1a_first_look.jpg", p1a_first_look, width = 10, height = 7)


p1a_first_look <- ggplot(data = filter(first_looks_df) %>%
                           mutate(Experiment = case_when(experiment_num == "pilot_1a" ~ "Pilot 1 Kids",
                                           experiment_num == "pilot_1a_adult" ~ "Pilot 1 Adults",
                                           experiment_num == "pilot_1b_no_outcome" ~ "Pilot 2",
                                           experiment_num == "pilot_1b_outcome" ~ "Pilot 2"),
                             trial_num_factor = as.numeric(as.factor(trial_num_factor)),
                             outcome = ifelse(grepl("no_outcome", experiment_num), "no outcome", "outcome")) %>%
                           group_by(trial_num_factor, Experiment, outcome) %>%
                           summarise(target=sum(aoi == "target")/(sum(aoi == "target") + 
                                                                    sum(aoi == "distractor"))) ,
                         aes(x = trial_num_factor, y = target, group = outcome, colour=outcome)
                         ) + 
  geom_line() + 
  geom_point() + 
  ylim(0, 1) + 
  labs(title = "First Look") +
  ylab("Proportion First Looks to Target")  +
  xlab("Trial Number") + 
  geom_hline(yintercept = 0.5, colour='gray') +
  facet_grid(. ~ Experiment)
ggsave(filename = "first_look_trial_number.png", p1a_first_look, width = 5.5, height = 3)
...
filter(mutate(df_analysis_windowed, id=paste(subid, trial_num)), experiment_num == 'pilot_1a') %>%
  ggplot(data=., aes(x=t, y=id, colour=aoi)) +
    geom_point(size=.1) +
  theme_bw(14) + 
  theme(axis.text.y = element_blank()) + 
  scale_colour_manual(values=c("blue", "gray", "red")) +
  ylab("trial") + 
  xlab("time (ms relative to disambiguation point)")
ggsave("dotmap.png", height=7, width=5)
```
...
prop_looking =  filter(df_analysis_windowed, aoi %in% c("target", "distractor")) %>%
                           mutate(Experiment = case_when(experiment_num == "pilot_1a" ~ "Pilot 1 Kids",
                                           experiment_num == "pilot_1a_adult" ~ "Pilot 1 Adults",
                                           experiment_num == "pilot_1b_no_outcome" ~ "Pilot 2",
                                           experiment_num == "pilot_1b_outcome" ~ "Pilot 2"),
                             trial_num_factor = trial_num,
                             outcome = ifelse(grepl("no_outcome", experiment_num), "no outcome", "outcome")) %>%
                           group_by(trial_num_factor, Experiment, outcome, lab, subid) %>%
                           summarise(group_target=mean(aoi == "target")) %>%
                           ungroup() %>%
                           group_by(trial_num_factor, Experiment, outcome) %>%
                                    summarise(target=mean(group_target))
ggplot(data = prop_looking, aes(x = trial_num_factor, y = target, group = outcome, colour=outcome)) + 
  geom_line() + 
  geom_point() + 
  ylim(0, 1) + 
  labs(title = "Proportion Looking") +
  ylab("Proportion Looking to Target")  +
  xlab("Trial Number") + 
  geom_hline(yintercept = 0.5, colour='gray') +
  facet_grid(. ~ Experiment)
ggsave("pro_looking_trialnum.png", width=5.5, height=3)
...
mutate(ms_proportion_each_trial,
         exp = case_when(grepl("1b", experiment_num) ~ "1b",
                         grepl("adult", experiment_num) ~ "adult",
                         TRUE ~ "1a")) %>%
  group_by(exp) %>%
  summarise(m=mean(proportion_target),
            se = sd(proportion_target)/sqrt(n()),
            lower = m - 1.96*se,
            upper = m + 1.96*se)
