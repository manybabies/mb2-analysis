...
subjs = dir(here(lab_dir, "raw_data/adults/"))
...
d.pretrim <- subjs %>%
  map_df(function(subj) {
    xy = edf.samples(paste0(here(lab_dir, "raw_data/adults/"), subj, "/", subj, ".edf"), trials=T) %>%
      mutate(lab_subject_id = subj)
    msg = edf.messages(paste0(here(lab_dir, "raw_data/adults/"), subj, "/", subj, ".edf"))
...
p <- readxl::read_xlsx(here(lab_dir, "raw_data/MB2_pilot_KU_CPH_participantsheet_adults.xlsx"))
...
datasets <- tibble(dataset_id = 10, 
                   ...
                   sample_rate = 500, 
                   tracker = "eyelink", 
                   lab_dataset_id = "copenhagen_babylab_adults")
