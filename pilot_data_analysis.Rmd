...
p1a_first_look <- ggplot(data = filter(first_looks_df, experiment_num == "pilot_1a") %>%
                           mutate(trial_num_factor = as.numeric(as.factor(trial_num_factor))) %>%
                           group_by(trial_num_factor) %>%
                           summarise(target=mean(aoi == "target"),
                                     distractor=mean(aoi == "distractor"),
                                     other=mean(aoi == "other")) %>%
                           gather(variable, value, -trial_num_factor),
                         aes(x = trial_num_factor, y = value, group = variable, colour=variable)
                         ) + 
  geom_line() + 
  geom_point() + 
  ylim(0, 1) + 
...
ggsave(filename = "1a_first_look.jpg", p1a_first_look, width = 4, height = 3)
...
filter(ungroup(df_analysis_windowed), experiment_num == "pilot_1a") %>%
  group_by(trial_num) %>%
  summarise(target=mean(aoi == "target"),
            distractor = mean(aoi == "distractor"),
            other = mean(aoi == "other"))  %>%
  gather(variable, value, -trial_num) %>%
  ggplot(data = .,
       aes(x=trial_num, y=value, group=variable, colour=variable)) + 
  geom_line() + 
  geom_point() + 
  ylim(0, 1) + 
  labs(title = "Pilot 1 (Children) Proportion Looking") +
  ylab("Proportion")  +
        ggthemes::scale_color_solarized(name = "Area of Interest")   +
  xlab("Trial Number") +
  theme(legend.position="bottom")
  
ggsave("pilot1_kids_trialnum.png", width=4, height=3)
