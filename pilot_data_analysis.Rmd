...
df_t_first_aoi <- d %>% 
  ...
  # create variable for AOI in order to exploit rle, which handles NaN's neatly
    mutate(aoi = ifelse(aoi == "other", NA, aoi)) %>%
  group_by(subid, trial_num) %>% 
    # 4 consecutive timepoints looking at target or distractor is the first look
  mutate(first_look_aoi = rle(aoi)$values[rle(aoi)$lengths >= 4][1],
   # get t at which first look to target or distractor begins
         first_look_t = t[which.max(rle(aoi)$lengths >= 4)]) %>% 
  ungroup()
