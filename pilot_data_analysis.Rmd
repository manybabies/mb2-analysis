...
## Set up priors for Bayesian analyses below. We are using a weakly informed prior for all analyses
```{r}
...
```{r}
ms_proportion_trials <- df_analysis2 %>%
  group_by(t, trial_num, experiment_num) %>%
  summarise(target = mean(aoi == "target", na.rm = TRUE),
            distractor = mean(aoi == "distractor", na.rm = TRUE)) %>%
...
```{r}
ms_proportion_trials_MLM <- df_analysis2 %>%
  ...
  mutate(p = mean_target / (mean_target + mean_distractor)) %>% 
  ungroup()


ggplot(data = ms_proportion_trials_MLM, aes(x = p))+
  geom_histogram()+
  facet_grid(.~experiment_num)#we have a lot of proportion clustering at 1 (meaning participants look at target often)

ggplot(data = ms_proportion_trials_MLM, aes(x = log(p)))+
  geom_histogram()+
  facet_grid(.~experiment_num)+
  ggtitle(label = "log-transformed p")#let us examine if we can log-transformed p
