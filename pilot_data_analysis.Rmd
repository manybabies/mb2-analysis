...
```{r include=FALSE}
#Mixed effect model for pilot 1a (kids) - full model (weaker info prior)
m_lmer_full_weaker_1a <- brm(proportion_target_c ~ trial_num_recoded + (trial_num_recoded | lab) + (trial_num_recoded | subid),
                   data=df_ms_proportion_1a, 
                   family = gaussian(), 
                           prior = priors_weaker,
                          save_pars = save_pars(all = TRUE),
...
```{r include = FALSE}
#Null model pilot 1a (kid) - (weaker info prior)
m_lmer_null_weaker_1a <- brm(proportion_target_c ~ 0 + trial_num_recoded + (trial_num_recoded | lab) + (trial_num_recoded | subid),
                   data=df_ms_proportion_1a, 
                   family = gaussian(), 
                           prior = null_prior_weaker,
                          save_pars = save_pars(all = TRUE),
...
```{r include = FALSE}
prop_marloglik_full_weaker_pilot1a <- bridge_sampler(m_lmer_full_weaker_1a, silent = T)

prop_marloglik_null_weaker_pilot1a <- bridge_sampler(m_lmer_null_weaker_1a, silent = T)
```
...
```{r include = FALSE}
BF_prop_In_weaker_info_1a <- bayes_factor(prop_marloglik_full_weaker_pilot1a, prop_marloglik_null_weaker_pilot1a)

BF_prop_In_weaker_info_1a
```
...
m_lmer_full_1b_o <- brm(proportion_target_c ~ trial_num_recoded + (trial_num_recoded | lab) + (trial_num_recoded | subid),
...
m_lmer_null_1b_o <- brm(proportion_target_c ~ 0 + trial_num_recoded + (trial_num_recoded | lab) + (trial_num_recoded | subid),
                   data=df_ms_proportion_1b_o, 
                   family = gaussian(), 
                           prior = null_prior,
                          save_pars = save_pars(all = TRUE),
...
Calculate the marginal log likelihoods for each hypothese using Bridge sampling
```{r}
prop_marloglik_full_pilot1b_o <- bridge_sampler(m_lmer_full_1b_o, silent = T)

prop_marloglik_null_pilot1b_o <- bridge_sampler(m_lmer_null_1b_o, silent = T)
```

Bayes factor
```{r}
BF_prop_In_more_info_1b_o <- bayes_factor(prop_marloglik_full_pilot1b_o, prop_marloglik_null_pilot1b_o)
