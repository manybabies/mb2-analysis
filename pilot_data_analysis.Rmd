...
d %>%
  ...
  ungroup() %>%
  group_by(lab) %>% 
  summarise(longest_trial=max(time_range),
            shortest_trial=min(time_range),
            median_trial=median(time_range),
            proportion_trials_at_median = mean(time_range == median_trial)) %>%
  kable(digits=2)
```
