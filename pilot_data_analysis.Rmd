...
df_t_first_aoi <- d %>% 
  ...
  filter(., experiment_num == "pilot_1a",
         t >= -4000+120, t <= 120) %>% 
  # create dummy variable for AOI in order to exploit rle, which handles NaN's neatly
   mutate(aoi_dummy = case_when(aoi == "other" ~ NaN, aoi == "target" ~ 1, aoi == "distractor" ~ 0)) %>%
  group_by(subid, trial_num) %>% 
    # 4 consecutive timepoints looking at target or distractor is counted as first look
  mutate(first_look_aoi_dummy = rle(aoi_dummy)$values[rle(aoi_dummy)$lengths >= 4][1]) %>% 
   # convert back to  strings
  mutate(first_look_aoi = case_when(first_look_aoi_dummy == 1 ~ "target", first_look_aoi_dummy == 0 ~ "distractor", is_na(first_look_aoi_dummy) ~ 'NA')) %>% 
