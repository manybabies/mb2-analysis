...
ms_timecourse <- df_analysis_windowed %>%
  filter(experiment_num == "pilot_1a", 
         lab == "uchicago_babylab") %>%
  mutate(target = as.numeric(aoi == "target"),
         trial_id = as.character(trial_id))
         
ops <- poly(unique(ms_timecourse$t), POLY_DEG)
ops_tibble <- tibble(ot1 = ops[,1], 
                     ot2 = ops[,2],
                     # ot3 = ops[,3], 
                     t = unique(ms_timecourse$t))
...
```{r}
m.full <- glmer(target ~ (ot1 + ot2) + 
                  (ot1 + ot2 | trial_id) +
                  (ot1 + ot2 | subid), 
                family = "binomial", 
                data = ms_timecourse)

ms_timecourse$fit <- fitted(m.full)
```

```{r}
ggplot(ms_timecourse, aes(x = t, y = target)) + 
